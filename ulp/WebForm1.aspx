...
<html>
  <head>
    ...
    function OnSubmit()
    {
        ...
        document.Main.action = "WebForm1.aspx";
        return true;        
    }
    </script>
	<STYLE>
  ...
  <body MS_POSITIONING="GridLayout">
    ...
    <%
        ...
        string ssql = "select distinct p.kodepokja, nama from pokja p left join aktorpokja a on p.KODEPOKJA=a.KODEPOKJA where p.kodepokja <> 'PJ.GD' and TAHUN = YEAR(getdate());";
    ...
    Pilih Pokja : <Custom:MultiSelectCombo id="Languege" Name="Languege" MaxSize="2" MaxListHeight="4" runat="server"></Custom:MultiSelectCombo>
    ...
    {
    %>
    <label id="selected_lang" name="selected_lang">POKJA terpilih : <%=sValues%></label><br/>
    <%}%>

    <input type="submit" value="Simpan Pokja" onclick="OnSubmit()">
	...
	{
	%>
    <input type="submit" value="Back" onclick="location.href='surat_tugas.aspx?usrgrp=<%=Request["usrgrp"]%>';">
	<%
	}
	if (Request["hid_selected_usrgrp"] != null && Request["hid_selected_usrgrp"] != "")
	{
	%>
    <input type="submit" value="Back" onclick="location.href='surat_tugas.aspx?<%=Request["hid_selected_usrgrp"]%>';">
	<%
	}
    if (Request["hid_selected_lang"] != null && Request["hid_selected_lang"] != "")
    {
	if (Request["nip"] != null && Request["nip"] != "")
	{
	%>
    <input type="submit" value="Next" onclick="location.href='WebForm2.aspx?nip=<%=Request["nip"]%>&usrgrp=<%=Request["usrgrp"]%>';">
	<%
	}
	else
	{	
	%>
    <input type="submit" value="Next" onclick="location.href='WebForm2.aspx?nip=<%=Request["hid_selected_lang"]%>&<%=Request["hid_selected_usrgrp"]%>';">
    <%
	}
	}
	%>
