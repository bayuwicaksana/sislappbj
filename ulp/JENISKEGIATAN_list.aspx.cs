...
public partial class CJENISKEGIATAN_List : AspNetRunnerPage 
{
    ...
    Data.JENISKEGIATANController controller = new Data.JENISKEGIATANController();
    Data.JENISKEGIATANCollection collection;

    Data.PBJCollection PBJ_detailCollection;
    Data.KELENGKAPANCollection KELENGKAPAN_detailCollection;

    protected void Page_Init( object sender,  System.EventArgs e)  
    {
        strTableName = "dbo.JENISKEGIATAN";
        strTableNameLocale = "dbo_JENISKEGIATAN";
    }

    protected void Page_Load( object sender,  System.EventArgs e)  
    {
        string output = string.Empty;
                        if (User != null && !string.IsNullOrEmpty("JENISKEGIATAN"))
            {
                OwnerColumn = func.GetOwnerIDField("JENISKEGIATAN");
            }
                        allow_PBJ = func.CheckUserPermissions("dbo.PBJ", "SA");
            ...
            output = func.BuildOutput(this, @"~\JENISKEGIATAN_list.aspx", smarty);
    ...
    private void AddDisplayGridAttributes()
    {
        if(display_grid)
        {
            ...
            grid_block["begin"] = "<form method=\"POST\" action=\"JENISKEGIATAN_list.aspx\" name=\"frmAdmin\" id=\"frmAdmin\">" +
        ...
        smarty.Add("recordspp_attrs","onchange=\"javascript: document.location='JENISKEGIATAN_list.aspx?pagesize='+this.options[this.selectedIndex].value;\"");
    ...
    private void AddLinkAttributes()
    {
	    ...
	    smarty.Add("printlink_attrs","onclick=\"window.open('JENISKEGIATAN_print.aspx','wPrint');return false;\"");
	    smarty.Add("printalllink_attrs","onclick=\"window.open('JENISKEGIATAN_print.aspx?all=1','wPrint');return false;\"");
	    smarty.Add("export_link", allow_export);
	    smarty.Add("exportlink_attrs","onclick=\"window.open('JENISKEGIATAN_export.aspx','wExport');return false;\"");
		...
		"return true;" +
	    "document.forms.frmAdmin.action='JENISKEGIATAN_print.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='JENISKEGIATAN_list.aspx'; " +
	    ...
	    "if(!$('input[@type=checkbox][@checked][@name^=selection]').length)" +
		    "return true;" +
	    "document.forms.frmAdmin.action='JENISKEGIATAN_export.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='JENISKEGIATAN_list.aspx';  " +
	    "document.forms.frmAdmin.target='_self';\"");
	    ...
	    smarty.Add("addlink_attrs","onClick=\"window.location.href='JENISKEGIATAN_add.aspx'\"");
	    smarty.Add("inlineaddlink_attrs","href=\"JENISKEGIATAN_add.aspx\" onclick=\"return inlineAdd(newrecord_id++);\"");
	    ...
	    smarty.Add("view_column",allow_search);
 	        ...
 	        smarty.Add("PBJ_dtable_column", allow_PBJ);
 	        smarty.Add("KELENGKAPAN_dtable_column", allow_KELENGKAPAN);
        ...
        smarty.Add("asearchlink_attrs","onclick=\"window.location.href='JENISKEGIATAN_search.aspx';return false;\"");
        smarty.Add("import_link",allow_import);
        smarty.Add("importlink_attrs","onclick=\"window.location.href='JENISKEGIATAN_import.aspx';return false;\"");
    ...
    private void BuildForm()
    {
	    ...
	    for(int i = 0; i < collection.Count; ++ i)
        {
                        ...
                        if(beforeProcessRowListResult)
            {
			    ...
			    masterquery = "mastertable=JENISKEGIATAN";
			    masterquery += "&masterkey1=" + this.Server.UrlEncode(collection[i].KODEJENISKEGIATAN.ToString());
                masterID = collection[i].KODEJENISKEGIATAN.ToString();
                //detail tables
	            record["PBJ_dtable_link"]=true;
	            //record["PBJ_dtablelink_attrs"]="onmouseover=\"RollDetailsLink.showPopup(this,'PBJ_detailspreview.aspx'+this.href.substr(this.href.indexOf('?')));\" onmouseout=\"RollDetailsLink.hidePopup();\" href=\"PBJ_list.aspx?" + masterquery + "\" id=\"master_PBJ" + recno + "\"";

                dpreview=true;
                int PBJ_cnt = 0;
                foreach (Data.PBJ details in PBJ_detailCollection)
                {
                    if (masterID != null && details.KODEJENISKEGIATAN.ToString() == masterID)
                    {
                        ++PBJ_cnt;
                    }
                }
	            	            if(PBJ_cnt > 0)
                {
		            record["PBJ_childcount"]=true;
                }
	            else
                {
		            dpreview=false;
                }
	            record["PBJ_childnumber"]=PBJ_cnt;
	                	        
                record["PBJ_dtablelink_attrs"]="href=\"PBJ_list.aspx?" + masterquery + "\" id=\"master_PBJ" + recno.ToString() + "\"";
                if(dpreview)
	            {
	            		            record["PBJ_dtablelink_attrs"] +=" onmouseover=\"RollDetailsLink.showPopup(this,'PBJ_detailspreview.aspx'+this.href.substr(this.href.indexOf('?')));\" onmouseout=\"RollDetailsLink.hidePopup();\"";
	                            }
			    masterquery = "mastertable=JENISKEGIATAN";
			    masterquery += "&masterkey1=" + this.Server.UrlEncode(collection[i].KODEJENISKEGIATAN.ToString());
                masterID = collection[i].KODEJENISKEGIATAN.ToString();
                //detail tables
	            record["KELENGKAPAN_dtable_link"]=true;
	            //record["KELENGKAPAN_dtablelink_attrs"]="onmouseover=\"RollDetailsLink.showPopup(this,'KELENGKAPAN_detailspreview.aspx'+this.href.substr(this.href.indexOf('?')));\" onmouseout=\"RollDetailsLink.hidePopup();\" href=\"KELENGKAPAN_list.aspx?" + masterquery + "\" id=\"master_KELENGKAPAN" + recno + "\"";

                dpreview=true;
                int KELENGKAPAN_cnt = 0;
                foreach (Data.KELENGKAPAN details in KELENGKAPAN_detailCollection)
                {
                    if (masterID != null && details.KODEJENISKEGIATAN.ToString() == masterID)
                    {
                        ++KELENGKAPAN_cnt;
	            	            ...
	            	            if(KELENGKAPAN_cnt > 0)
                {
		            record["KELENGKAPAN_childcount"]=true;
                }
	            else
                {
		            dpreview=false;
                }
	            record["KELENGKAPAN_childnumber"]=KELENGKAPAN_cnt;
	                	        
                record["KELENGKAPAN_dtablelink_attrs"]="href=\"KELENGKAPAN_list.aspx?" + masterquery + "\" id=\"master_KELENGKAPAN" + recno.ToString() + "\"";
                if(dpreview)
	            {
	            		            record["KELENGKAPAN_dtablelink_attrs"] +=" onmouseover=\"RollDetailsLink.showPopup(this,'KELENGKAPAN_detailspreview.aspx'+this.href.substr(this.href.indexOf('?')));\" onmouseout=\"RollDetailsLink.hidePopup();\"";
	                            }
            ...
            //	key fields
	            ...
	            string copylink="";
	            string keylink="";
	            keyblock +=  this.Server.UrlEncode(collection[i].KODEJENISKEGIATAN.ToString());
	            editlink += "editid1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEJENISKEGIATAN.ToString()));
	            copylink +="copyid1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEJENISKEGIATAN.ToString()));
	            keylink +="&key1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEJENISKEGIATAN.ToString()));

	            record["editlink_attrs"]="href=\"JENISKEGIATAN_edit.aspx?" + editlink + "\" id=\"editlink" + recno.ToString() + "\"";
	            record["inlineeditlink_attrs"]= "href=\"JENISKEGIATAN_edit.aspx?" + editlink + "\" onclick=\"return inlineEdit('" + recno.ToString() + "','" + editlink + "');\" id=\"ieditlink" + recno.ToString() + "\"";
	            record["copylink_attrs"]="href=\"JENISKEGIATAN_add.aspx?" + copylink + "\" id=\"copylink" + recno.ToString() + "\"";
	            record["viewlink_attrs"]="href=\"JENISKEGIATAN_view.aspx?"  + editlink + "\" id=\"viewlink" + recno.ToString() + "\"";
    ...
    private void GetData()
    {
        ...
        {
            collection = new Data.JENISKEGIATANCollection();
            numrows = 0;
        }
        else
        {
        ...
        Data.PBJController PBJ_detailController = new Data.PBJController();
        //foreach @TABLE.arrKeyFields as @tk
                ArrayList PBJ_masterids = new ArrayList();
        for(int i = 0; i < collection.Count; ++ i)
        {
            try{
            PBJ_masterids.Add(collection[i].KODEJENISKEGIATAN.ToString());
            }
            catch
            {
            }
        }
        PBJ_detailCollection = PBJ_detailController.FetchForDetails("KODEJENISKEGIATAN", PBJ_masterids.ToArray(), OwnerColumn, OwnerID);
        ...
        {
            try{
            KELENGKAPAN_masterids.Add(collection[i].KODEJENISKEGIATAN.ToString());
            }
            catch
            {
            }
        }
        KELENGKAPAN_detailCollection = KELENGKAPAN_detailController.FetchForDetails("KODEJENISKEGIATAN", KELENGKAPAN_masterids.ToArray(), OwnerColumn, OwnerID);
        ...
        if(allow_search)
        {
	        ...
	        else
	        {
	            ...
	            if(maxpages > 1)
	            {
                    ...
                    pagination += "{window.location='JENISKEGIATAN_list.aspx?goto=' + nPageNumber;}</script>";
        ...
        private void DeleteRecord()
    {
        ...
        foreach(string val in selected_recs)
        {
                        ...
                        if(!abortDeleting)
            {
                ...
                {
                                    Data.JENISKEGIATAN.Destroy(val);
                }
                            }
        }
    ...
    private void ProcessSessionVariables()
    {
        ...
        smarty.Add("KODEJENISKEGIATAN_orderlinkattrs", "href=\"JENISKEGIATAN_list.aspx?orderby=aKODEJENISKEGIATAN\"");
        smarty.Add("KODEJENISKEGIATAN_fieldheader", true);
        smarty.Add("DESKRIPSI_orderlinkattrs", "href=\"JENISKEGIATAN_list.aspx?orderby=aDESKRIPSI\"");
        ...
        if(tmp_orderby != null)
        {
	        ...
	        smarty[Control.GoodFieldName(order_field) + "_orderlinkattrs"] = "href=\"JENISKEGIATAN_list.aspx?orderby=" + dir + Control.GoodFieldName(order_field) + "\"";
    ...
    private void Body()
    {
        ...
        "<form name=\"frmSearch\" method=\"GET\" action=\"JENISKEGIATAN_list.aspx\">" + 
    ...
    private string GetIncludes()
    {
	    ...
	    includes.Append("var INLINE_EDIT_TABLE='JENISKEGIATAN_edit.aspx';\r\n");
	    includes.Append("var INLINE_ADD_TABLE='JENISKEGIATAN_add.aspx';\r\n");
	    includes.Append("var INLINE_VIEW_TABLE='JENISKEGIATAN_view.aspx';\r\n");
	    includes.Append("var SUGGEST_TABLE='JENISKEGIATAN_searchsuggest.aspx';\r\n");
	    includes.Append("var MASTER_PREVIEW_TABLE='JENISKEGIATAN_masterpreview.aspx';\r\n");
        includes.Append("\n</script>\n");
