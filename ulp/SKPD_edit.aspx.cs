...
public partial class CSKPD_Edit : AspNetRunnerPage 
{
    ...
    private void SaveData()
    {
        ...
        Data.SKPD emptyItem = new Data.SKPD();
        ...
        //
        //item.KODESKPD =  Convert.ToString(Request["editid1"]);
        //
        string idx = string.Empty;
        idx = "1";
                                               item.KODESKPD =  Convert.ToString(Request["editid" + idx]);
    ...
    private void BuildForm()
    {
        ...
        string keylink="";
	    keylink +="&key1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(item.KODESKPD.ToString()));

        if(RequestAction == "edited" && inlineedit)
        {
            string masterquery = string.Empty;
	        masterquery="mastertable=dbo%2ESKPD";
	        masterquery += "&masterkey1=" + this.Server.UrlEncode(item.KODESKPD.ToString());
	        showDetailKeys["PBJ"] = masterquery;
        	
	        showKeys.Add(Control.HTMLEncodeSpecialChars(item.KODESKPD.ToString()));
    ...
    private void BuildBody()
    {
		...
		body["begin"] = includes + 
		"<form name=\"editform\" encType=\"multipart/form-data\" method=\"post\" action=\"SKPD_edit.aspx\" " + onsubmit + ">" +
        ...
        body["begin"] +="<input type=\"hidden\" name=\"editid1\" value=\""+ Control.HTMLEncodeSpecialChars(keys["KODESKPD"].ToString()) + "\">";
	    	    smarty.Add("show_key1", Control.HTMLEncodeSpecialChars(control_KODESKPD.GetData()));

        smarty.Add("backbutton_attrs","onclick=\"window.location.href='SKPD_list.aspx?a=return'\"");
	    smarty.Add("save_button",true);
	    smarty.Add("reset_button",true);
	    smarty.Add("back_button",true);
	    
        showKeys.Add(Server.UrlEncode(keys["KODESKPD"].ToString()));
    }
    ...
    private string GetIncludes()
    {
        ...
        if ( !inlineedit ) 
        {
	        ...
	        includes.Append( "</script>\r\n");
		        ...
		        {
			        bodyonload += "define('value_KODESKPD','" + validatetype + "','KODE SKPD');";
		        }
    ...
    private bool Init()
    {
        ...
        if(inlineedit)
        {
	        templatefile = "SKPD_inline_edit.aspx";
        }
        else
        {
	        templatefile = "SKPD_edit.aspx";
        }

        keys["KODESKPD"] = Request["editid1"];
        if(keys.Count > 1)
        {
        ...
        {
            item = Data.SKPD.FetchByID(Request["editid1"]);
        }
        ...
        if(item != null)
        {
            ...
            if(!editable)
            {
                output.Append("<p>" + "You don't have permissions to access this table" + " <a href=\"SKPD_list.aspx?a=return\">back</a>");
                return false;
            }
        }
        else
        {
            this.Server.Transfer("~/SKPD_list.aspx?a=return");
        }
