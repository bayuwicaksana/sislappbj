...
public partial class CAKTOR_Add : AspNetRunnerPage 
{
    ...
    private void SaveData()
    {
        item = new Data.AKTOR();
                //	processing NIP - start
                        if(!string.IsNullOrEmpty(Request["value_NIP"]))
        {
                                                                                                item.NIP = Convert.ToString(Request["value_NIP"]);
                    }
                        ...
                        if(!string.IsNullOrEmpty(Request["value_NAMA"]))
        {
                ...
                //	processing KODEJABATAN - start
                        if(!string.IsNullOrEmpty(Request["value_KODEJABATAN"]))
        {
                                                                                                item.KODEJABATAN = Convert.ToString(Request["value_KODEJABATAN"]);
                    }
        ...
        //      insert masterkey value if exists and if not specified
           if(Mastertable =="dbo.TIPEAKTOR")
           {
                   if(string.IsNullOrEmpty(Request["value_KODETIPE"]))
                   {

                                           item.KODETIPE =  Convert.ToString(Session[strTableName + "_masterkey1"]);
                        }
           }
           if(Mastertable =="dbo.JABATANAKTOR")
           {
                   if(string.IsNullOrEmpty(Request["value_KODEJABATAN"]))
                   {

                                           item.KODEJABATAN =  Convert.ToString(Session[strTableName + "_masterkey1"]);
                        }
           }
    ...
    private void BuildForm()
    {
        ...
        {
            string masterquery="";
	        masterquery="mastertable=dbo%2EAKTOR";
	        masterquery += "&masterkey1=" + this.Server.UrlEncode(item.NIP.ToString());
	        showDetailKeys["dbo.PBJ"] = masterquery;
	        masterquery="mastertable=dbo%2EAKTOR";
	        masterquery += "&masterkey1=" + this.Server.UrlEncode(item.NIP.ToString());
        ...
        else
        {
            ...
            object NIP_value = null;
            if(defvalues.ContainsKey("NIP"))
            {
                NIP_value = defvalues["NIP"];
            }
            else if(isCopy)
            {
                NIP_value = item.NIP;
            ...
            Control edit_control_NIP = null;

            if(!string.IsNullOrEmpty(Mastertable) && (detailkeys.ContainsKey("NIP")))
            {
                edit_control_NIP = new Control("NIP", detailkeys["NIP"], false, smarty, this.Request, builder, ctrlMode);
            }
            else
            {
                                edit_control_NIP = new Control("NIP", NIP_value, false, smarty, this.Request, builder, ctrlMode);
            }

                                    smarty.Add("NIP_editcontrol", edit_control_NIP.BuildEditControl());
            ...
            object KODEJABATAN_value = null;
            if(defvalues.ContainsKey("KODEJABATAN"))
            {
                KODEJABATAN_value = defvalues["KODEJABATAN"];
            }
            else if(isCopy)
            {
                KODEJABATAN_value = item.KODEJABATAN;
            }
            if(inlineedit == ADD_MODE.ADD_INLINE)
            {
                ctrlMode = MODE.MODE_INLINE_ADD;
            ...
            Control edit_control_KODEJABATAN = null;

            if(!string.IsNullOrEmpty(Mastertable) && (detailkeys.ContainsKey("KODEJABATAN")))
            {
                edit_control_KODEJABATAN = new Control("KODEJABATAN", detailkeys["KODEJABATAN"], false, smarty, this.Request, builder, ctrlMode);
            }
            else
            {
                                edit_control_KODEJABATAN = new Control("KODEJABATAN", KODEJABATAN_value, false, smarty, this.Request, builder, ctrlMode);
            }

                        func.PopulateLookupFields(edit_control_KODEJABATAN.FieldInfo);
                        smarty.Add("KODEJABATAN_editcontrol", edit_control_KODEJABATAN.BuildEditControl());
            ...
            else
            {
                        ...
                        func.PopulateLookupFields(edit_control_KODETIPE.FieldInfo);
    ...
    private void CopyRecord()
    {
        ...
        if(Request["copyid1"] != null || Request["editid1"] != null)
        {
	        ...
	        {
                item = Data.AKTOR.FetchByID(Request["copyid1"]);
                isCopy = true;
	        }
	        else
	        {
                 item = Data.AKTOR.FetchByID(Request["editid1"]);
	        }

                        //	clear key fields
	            defvalues["NIP"] = item.NIP;
        }
    ...
    private void BuildBody()
    {
	    ...
	    if(inlineedit != ADD_MODE.ADD_ONTHEFLY)
	    {
		    ...
		    body["begin"] = includes + 
		    "<form name=\"editform\" encType=\"multipart/form-data\" method=\"post\" action=\"AKTOR_add.aspx\" " + onsubmit + ">" +
		    "<input type=hidden name=\"a\" value=\"added\">";
		    smarty.Add("backbutton_attrs","onclick=\"window.location.href='AKTOR_list.aspx?a=return'\"");
		    smarty.Add("back_button",true);
	    }
	    else
	    {
		    formname = "editform" + (string)Request["id"];
		    body["begin"]= "<form name=\"editform" + (string)Request["id"] + "\" encType=\"multipart/form-data\" method=\"post\" action=\"AKTOR_add.aspx\" " + onsubmit + " target=\"flyframe" + (string)Request["id"] + "\">" +
		    "<input type=hidden name=\"a\" value=\"added\">" + 
        ...
        if ( inlineedit != ADD_MODE.ADD_INLINE ) 
        {
            ...
            else
            {
		        ...
		        if(!string.IsNullOrEmpty(validatetype))
		        {
			        ...
			        if(inlineedit == ADD_MODE.ADD_ONTHEFLY)
                    {
				        linkdata += "define_fly('value_NIP_" + (string)Request["id"] + "','" + validatetype + "');";
                    }
			        else
                    {
				        bodyonload += "define('value_NIP','" + validatetype + "','NIP');";
                    }
		        }
    ...
    private void Init()
    {
        ...
        if(inlineedit == ADD_MODE.ADD_INLINE)
        {
	        templatefile = "AKTOR_inline_add.aspx";
        }
        else
        {
	        templatefile = "AKTOR_add.aspx";
        }
    }
