...
public partial class CSTATUSPBJ_List : AspNetRunnerPage 
{
    ...
    Data.STATUSPBJCollection collection;
    ...
    protected void Page_Load( object sender,  System.EventArgs e)  
    {
        string output = string.Empty;
                        if (User != null && !string.IsNullOrEmpty("STATUSPBJ"))
            {
                OwnerColumn = func.GetOwnerIDField("STATUSPBJ");
            }
            ...
            output = func.BuildOutput(this, @"~\STATUSPBJ_list.aspx", smarty);
    ...
    private void AddDisplayGridAttributes()
    {
        if(display_grid)
        {
            ...
            grid_block["begin"] = "<form method=\"POST\" action=\"STATUSPBJ_list.aspx\" name=\"frmAdmin\" id=\"frmAdmin\">" +
        ...
        smarty.Add("recordspp_attrs","onchange=\"javascript: document.location='STATUSPBJ_list.aspx?pagesize='+this.options[this.selectedIndex].value;\"");
    ...
    private void AddLinkAttributes()
    {
	    ...
	    smarty.Add("printlink_attrs","onclick=\"window.open('STATUSPBJ_print.aspx','wPrint');return false;\"");
	    smarty.Add("printalllink_attrs","onclick=\"window.open('STATUSPBJ_print.aspx?all=1','wPrint');return false;\"");
	    smarty.Add("export_link", allow_export);
	    smarty.Add("exportlink_attrs","onclick=\"window.open('STATUSPBJ_export.aspx','wExport');return false;\"");
		...
		"return true;" +
	    "document.forms.frmAdmin.action='STATUSPBJ_print.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='STATUSPBJ_list.aspx'; " +
	    ...
	    "if(!$('input[@type=checkbox][@checked][@name^=selection]').length)" +
		    "return true;" +
	    "document.forms.frmAdmin.action='STATUSPBJ_export.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='STATUSPBJ_list.aspx';  " +
	    "document.forms.frmAdmin.target='_self';\"");
	    ...
	    smarty.Add("addlink_attrs","onClick=\"window.location.href='STATUSPBJ_add.aspx'\"");
	    smarty.Add("inlineaddlink_attrs","href=\"STATUSPBJ_add.aspx\" onclick=\"return inlineAdd(newrecord_id++);\"");
        ...
        smarty.Add("KODESTATUS_fieldheadercolumn",true);
        smarty.Add("KODESTATUS_fieldcolumn",true);
        smarty.Add("KODESTATUS_fieldfootercolumn",true);
        ...
        smarty.Add("URUTAN_fieldheadercolumn",true);
        smarty.Add("URUTAN_fieldcolumn",true);
        smarty.Add("URUTAN_fieldfootercolumn",true);
        	
                display_grid = allow_search && rowsfound;

        smarty.Add("asearch_link",allow_search);
        smarty.Add("asearchlink_attrs","onclick=\"window.location.href='STATUSPBJ_search.aspx';return false;\"");
        smarty.Add("import_link",allow_import);
        smarty.Add("importlink_attrs","onclick=\"window.location.href='STATUSPBJ_import.aspx';return false;\"");
    ...
    private void BuildForm()
    {
	    ...
	    for(int i = 0; i < collection.Count; ++ i)
        {
                        ...
                        if(beforeProcessRowListResult)
            {
			    ...
			    masterquery = "mastertable=STATUSPBJ";
			    masterquery += "&masterkey1=" + this.Server.UrlEncode(collection[i].KODESTATUS.ToString());
                masterID = collection[i].KODESTATUS.ToString();
                //detail tables
                ...
                {
                    if (masterID != null && details.KODESTATUSPBJ.ToString() == masterID)
                    {
                        ++PBJ_cnt;
                    }
                }
            ...
            //	key fields
	            ...
	            string copylink="";
	            string keylink="";
	            keyblock +=  this.Server.UrlEncode(collection[i].KODESTATUS.ToString());
	            editlink += "editid1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODESTATUS.ToString()));
	            copylink +="copyid1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODESTATUS.ToString()));
	            keylink +="&key1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODESTATUS.ToString()));

	            record["editlink_attrs"]="href=\"STATUSPBJ_edit.aspx?" + editlink + "\" id=\"editlink" + recno.ToString() + "\"";
	            record["inlineeditlink_attrs"]= "href=\"STATUSPBJ_edit.aspx?" + editlink + "\" onclick=\"return inlineEdit('" + recno.ToString() + "','" + editlink + "');\" id=\"ieditlink" + recno.ToString() + "\"";
	            record["copylink_attrs"]="href=\"STATUSPBJ_add.aspx?" + copylink + "\" id=\"copylink" + recno.ToString() + "\"";
	            record["viewlink_attrs"]="href=\"STATUSPBJ_view.aspx?"  + editlink + "\" id=\"viewlink" + recno.ToString() + "\"";
	            record["checkbox_attrs"]="name=\"selection[]\" value=\"" + keyblock + "\" id=\"check" + recno.ToString() + "\"";

            string value;
            value = string.Empty;
    //	KODESTATUS - 
            Control control_KODESTATUS = new Control("KODESTATUS", collection[i].KODESTATUS, false, smarty, this.Request, builder, MODE.MODE_LIST);
	                        value = control_KODESTATUS.GetData();
			    value = control_KODESTATUS.ProcessLargeText(value,"field=KODESTATUS" + keylink,"",MODE.MODE_LIST);
			    record["KODESTATUS_value"]=value;
            value = string.Empty;
    //	DESKRIPSI - 
            ...
            value = string.Empty;
    //	URUTAN - 
            Control control_URUTAN = new Control("URUTAN", collection[i].URUTAN, false, smarty, this.Request, builder, MODE.MODE_LIST);
	                        value = control_URUTAN.GetData();
			    value = control_URUTAN.ProcessLargeText(value,"field=URUTAN" + keylink,"",MODE.MODE_LIST);
			    record["URUTAN_value"]=value;
			    record["show"]=true;
    ...
    private void GetData()
    {
        ...
        {
            collection = new Data.STATUSPBJCollection();
            numrows = 0;
        }
        else
        {
        ...
        {
            try{
            PBJ_masterids.Add(collection[i].KODESTATUS.ToString());
            }
            catch
            {
            }
        }
        PBJ_detailCollection = PBJ_detailController.FetchForDetails("KODESTATUSPBJ", PBJ_masterids.ToArray(), OwnerColumn, OwnerID);
        ...
        if(allow_search)
        {
	        ...
	        else
	        {
	            ...
	            if(maxpages > 1)
	            {
                    ...
                    pagination += "{window.location='STATUSPBJ_list.aspx?goto=' + nPageNumber;}</script>";
    ...
    private void ProcessSessionVariables()
    {
        ...
        smarty.Add("KODESTATUS_orderlinkattrs", "href=\"STATUSPBJ_list.aspx?orderby=aKODESTATUS\"");
        smarty.Add("KODESTATUS_fieldheader", true);
        smarty.Add("DESKRIPSI_orderlinkattrs", "href=\"STATUSPBJ_list.aspx?orderby=aDESKRIPSI\"");
        smarty.Add("DESKRIPSI_fieldheader", true);
        smarty.Add("URUTAN_orderlinkattrs", "href=\"STATUSPBJ_list.aspx?orderby=aURUTAN\"");
        smarty.Add("URUTAN_fieldheader", true);
        ...
        if(tmp_orderby != null)
        {
	        ...
	        smarty[Control.GoodFieldName(order_field) + "_orderlinkattrs"] = "href=\"STATUSPBJ_list.aspx?orderby=" + dir + Control.GoodFieldName(order_field) + "\"";
    ...
    private void Body()
    {
        ...
        "<form name=\"frmSearch\" method=\"GET\" action=\"STATUSPBJ_list.aspx\">" + 
    ...
    private string GetIncludes()
    {
	    ...
	    includes.Append("var INLINE_EDIT_TABLE='STATUSPBJ_edit.aspx';\r\n");
	    includes.Append("var INLINE_ADD_TABLE='STATUSPBJ_add.aspx';\r\n");
	    includes.Append("var INLINE_VIEW_TABLE='STATUSPBJ_view.aspx';\r\n");
	    includes.Append("var SUGGEST_TABLE='STATUSPBJ_searchsuggest.aspx';\r\n");
	    includes.Append("var MASTER_PREVIEW_TABLE='STATUSPBJ_masterpreview.aspx';\r\n");
        includes.Append("\n</script>\n");
