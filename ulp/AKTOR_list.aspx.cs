...
public partial class CAKTOR_List : AspNetRunnerPage 
{
    ...
    Data.AKTORController controller = new Data.AKTORController();
    Data.AKTORCollection collection;
    ...
    protected void Page_Load( object sender,  System.EventArgs e)  
    {
        string output = string.Empty;
                        if (User != null && !string.IsNullOrEmpty("AKTOR"))
            {
                OwnerColumn = func.GetOwnerIDField("AKTOR");
            }
			...
			output = func.BuildOutput(this, @"~\AKTOR_list.aspx", smarty);
    ...
    private void AddDisplayGridAttributes()
    {
        if(display_grid)
        {
            ...
            grid_block["begin"] = "<form method=\"POST\" action=\"AKTOR_list.aspx\" name=\"frmAdmin\" id=\"frmAdmin\">" +
        ...
        smarty.Add("recordspp_attrs","onchange=\"javascript: document.location='AKTOR_list.aspx?pagesize='+this.options[this.selectedIndex].value;\"");
    ...
    private void AddMasterDetailInfo()
    {
        ...
        //	include proper masterlist.aspx code
	        ...
	        string master = TIPEAKTORMasterList.CreateMasterTableInfo("AKTOR", keys, this);
	        smarty.Add("showmasterfile",master);
	        smarty.Add("backtomaster_link",true);
	        smarty.Add("mastertable_short","TIPEAKTOR");
        }
        ...
        //	include proper masterlist.aspx code
            string[] keys = new string[1];
            keys[1 - 1] = (string)this.Session[strTableName + "_masterkey1"];
	        string master = JABATANAKTORMasterList.CreateMasterTableInfo("AKTOR", keys, this);
	        smarty.Add("showmasterfile",master);
	        smarty.Add("backtomaster_link",true);
	        smarty.Add("mastertable_short","JABATANAKTOR");
        }
    }
    ...
    private void AddLinkAttributes()
    {
	    ...
	    smarty.Add("printlink_attrs","onclick=\"window.open('AKTOR_print.aspx','wPrint');return false;\"");
	    smarty.Add("printalllink_attrs","onclick=\"window.open('AKTOR_print.aspx?all=1','wPrint');return false;\"");
	    smarty.Add("export_link", allow_export);
	    smarty.Add("exportlink_attrs","onclick=\"window.open('AKTOR_export.aspx','wExport');return false;\"");
		...
		"return true;" +
	    "document.forms.frmAdmin.action='AKTOR_print.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='AKTOR_list.aspx'; " +
	    ...
	    "if(!$('input[@type=checkbox][@checked][@name^=selection]').length)" +
		    "return true;" +
	    "document.forms.frmAdmin.action='AKTOR_export.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='AKTOR_list.aspx';  " +
	    "document.forms.frmAdmin.target='_self';\"");
	    ...
	    smarty.Add("addlink_attrs","onClick=\"window.location.href='AKTOR_add.aspx'\"");
	    smarty.Add("inlineaddlink_attrs","href=\"AKTOR_add.aspx\" onclick=\"return inlineAdd(newrecord_id++);\"");
        ...
        smarty.Add("KODEJABATAN_fieldheadercolumn",true);
        smarty.Add("KODEJABATAN_fieldcolumn",true);
        smarty.Add("KODEJABATAN_fieldfootercolumn",true);
        ...
        smarty.Add("asearchlink_attrs","onclick=\"window.location.href='AKTOR_search.aspx';return false;\"");
        smarty.Add("import_link",allow_import);
        smarty.Add("importlink_attrs","onclick=\"window.location.href='AKTOR_import.aspx';return false;\"");
    ...
    private void BuildForm()
    {
	    ...
	    for(int i = 0; i < collection.Count; ++ i)
        {
                        ...
                        if(beforeProcessRowListResult)
            {
			    ...
			    masterquery = "mastertable=AKTOR";
			    masterquery += "&masterkey1=" + this.Server.UrlEncode(collection[i].NIP.ToString());
                masterID = collection[i].NIP.ToString();
                //detail tables
                ...
                {
                    if (masterID != null && details.PPTK.ToString() == masterID)
                    {
                        ++PBJ_cnt;
                    }
                }
                ...
                {
                    if (masterID != null && details.NIP.ToString() == masterID)
                    {
                        ++ASSIGNMENT_cnt;
                    }
                }
            ...
            //	key fields
	            ...
	            record["editlink_attrs"]="href=\"AKTOR_edit.aspx?" + editlink + "\" id=\"editlink" + recno.ToString() + "\"";
	            record["inlineeditlink_attrs"]= "href=\"AKTOR_edit.aspx?" + editlink + "\" onclick=\"return inlineEdit('" + recno.ToString() + "','" + editlink + "');\" id=\"ieditlink" + recno.ToString() + "\"";
	            record["copylink_attrs"]="href=\"AKTOR_add.aspx?" + copylink + "\" id=\"copylink" + recno.ToString() + "\"";
	            record["viewlink_attrs"]="href=\"AKTOR_view.aspx?"  + editlink + "\" id=\"viewlink" + recno.ToString() + "\"";
    ...
    //	NAMA - 
            ...
            value = string.Empty;
    //	KODEJABATAN - 
            Control control_KODEJABATAN = new Control("KODEJABATAN", collection[i].KODEJABATAN, false, smarty, this.Request, builder, MODE.MODE_LIST);
	                        //func.PopulateLookupFields(control_KODEJABATAN.FieldInfo);
			    value=func.GetLookupValue(control_KODEJABATAN.FieldInfo);
                control_KODEJABATAN.Value = value;
			    record["KODEJABATAN_value"]=value;
            value = string.Empty;
    //	KODETIPE - 
            Control control_KODETIPE = new Control("KODETIPE", collection[i].KODETIPE, false, smarty, this.Request, builder, MODE.MODE_LIST);
			    ...
			    value=func.GetLookupValue(control_KODETIPE.FieldInfo);
                control_KODETIPE.Value = value;
    ...
    private void GetAdvancedSearchRows()
    {
        ...
        if(Mastertable=="TIPEAKTOR")
        {
	        Asearchfor["KODETIPE"] = (string)this.Session[strTableName + "_masterkey1"];
            par.Add("KODETIPE", this.Session[strTableName + "_masterkey1"]);
        }
        if(Mastertable=="JABATANAKTOR")
        {
	        Asearchfor["KODEJABATAN"] = (string)this.Session[strTableName + "_masterkey1"];
            par.Add("KODEJABATAN", this.Session[strTableName + "_masterkey1"]);
        }
        string oCol = OwnerColumn;
        string oID = OwnerID;
    ...
    private void GetData()
    {
        ...
        {
            collection = new Data.AKTORCollection();
            numrows = 0;
        }
        else
        {
        ...
        {
            try{
            PBJ_masterids.Add(collection[i].NIP.ToString());
            }
            catch
            {
            }
        }
        PBJ_detailCollection = PBJ_detailController.FetchForDetails("PPTK", PBJ_masterids.ToArray(), OwnerColumn, OwnerID);
        ...
        {
            try{
            ASSIGNMENT_masterids.Add(collection[i].NIP.ToString());
            }
            catch
            {
            }
        }
        ASSIGNMENT_detailCollection = ASSIGNMENT_detailController.FetchForDetails("NIP", ASSIGNMENT_masterids.ToArray(), OwnerColumn, OwnerID);
        ...
        if(allow_search)
        {
	        ...
	        else
	        {
	            ...
	            if(maxpages > 1)
	            {
                    ...
                    pagination += "{window.location='AKTOR_list.aspx?goto=' + nPageNumber;}</script>";
    ...
    private void ProcessSessionVariables()
    {
        ...
        smarty.Add("NIP_orderlinkattrs", "href=\"AKTOR_list.aspx?orderby=aNIP\"");
        smarty.Add("NIP_fieldheader", true);
        smarty.Add("NAMA_orderlinkattrs", "href=\"AKTOR_list.aspx?orderby=aNAMA\"");
        smarty.Add("NAMA_fieldheader", true);
        smarty.Add("KODEJABATAN_orderlinkattrs", "href=\"AKTOR_list.aspx?orderby=aKODEJABATAN\"");
        smarty.Add("KODEJABATAN_fieldheader", true);
        smarty.Add("KODETIPE_orderlinkattrs", "href=\"AKTOR_list.aspx?orderby=aKODETIPE\"");
        ...
        if(tmp_orderby != null)
        {
	        ...
	        smarty[Control.GoodFieldName(order_field) + "_orderlinkattrs"] = "href=\"AKTOR_list.aspx?orderby=" + dir + Control.GoodFieldName(order_field) + "\"";
    ...
    private void Body()
    {
        ...
        "<form name=\"frmSearch\" method=\"GET\" action=\"AKTOR_list.aspx\">" + 
    ...
    private string GetIncludes()
    {
	    ...
	    includes.Append("var INLINE_EDIT_TABLE='AKTOR_edit.aspx';\r\n");
	    includes.Append("var INLINE_ADD_TABLE='AKTOR_add.aspx';\r\n");
	    includes.Append("var INLINE_VIEW_TABLE='AKTOR_view.aspx';\r\n");
	    includes.Append("var SUGGEST_TABLE='AKTOR_searchsuggest.aspx';\r\n");
	    includes.Append("var MASTER_PREVIEW_TABLE='AKTOR_masterpreview.aspx';\r\n");
        includes.Append("\n</script>\n");
        ...
        private void ProcessMasterTableOptions()
    {
        ...
        else
        {
            ...
            if(mtable=="TIPEAKTOR")
            {
                ...
                smarty.Add("backtomasterlink_attrs","href=\"TIPEAKTOR_list.aspx?a=return\"");
            }
            if(mtable=="JABATANAKTOR")
            {
                	            Mastertable = mtable;
                //	copy keys to session
	            int i = 1;
	            ...
	            smarty.Add("mastertable_block", true);
                smarty.Add("backtomasterlink_attrs","href=\"JABATANAKTOR_list.aspx?a=return\"");
            }
    }
