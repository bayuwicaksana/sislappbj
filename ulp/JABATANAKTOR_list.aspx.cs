...
public partial class CJABATANAKTOR_List : AspNetRunnerPage 
{
    ...
    Data.JABATANAKTORCollection collection;
    ...
    protected void Page_Load( object sender,  System.EventArgs e)  
    {
        string output = string.Empty;
                        if (User != null && !string.IsNullOrEmpty("JABATANAKTOR"))
            {
                OwnerColumn = func.GetOwnerIDField("JABATANAKTOR");
            }
            ...
            output = func.BuildOutput(this, @"~\JABATANAKTOR_list.aspx", smarty);
    ...
    private void AddDisplayGridAttributes()
    {
        if(display_grid)
        {
            ...
            grid_block["begin"] = "<form method=\"POST\" action=\"JABATANAKTOR_list.aspx\" name=\"frmAdmin\" id=\"frmAdmin\">" +
        ...
        smarty.Add("recordspp_attrs","onchange=\"javascript: document.location='JABATANAKTOR_list.aspx?pagesize='+this.options[this.selectedIndex].value;\"");
    ...
    private void AddLinkAttributes()
    {
	    ...
	    smarty.Add("printlink_attrs","onclick=\"window.open('JABATANAKTOR_print.aspx','wPrint');return false;\"");
	    smarty.Add("printalllink_attrs","onclick=\"window.open('JABATANAKTOR_print.aspx?all=1','wPrint');return false;\"");
	    smarty.Add("export_link", allow_export);
	    smarty.Add("exportlink_attrs","onclick=\"window.open('JABATANAKTOR_export.aspx','wExport');return false;\"");
		...
		"return true;" +
	    "document.forms.frmAdmin.action='JABATANAKTOR_print.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='JABATANAKTOR_list.aspx'; " +
	    ...
	    "if(!$('input[@type=checkbox][@checked][@name^=selection]').length)" +
		    "return true;" +
	    "document.forms.frmAdmin.action='JABATANAKTOR_export.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='JABATANAKTOR_list.aspx';  " +
	    "document.forms.frmAdmin.target='_self';\"");
	    ...
	    smarty.Add("addlink_attrs","onClick=\"window.location.href='JABATANAKTOR_add.aspx'\"");
	    smarty.Add("inlineaddlink_attrs","href=\"JABATANAKTOR_add.aspx\" onclick=\"return inlineAdd(newrecord_id++);\"");
        ...
        smarty.Add("asearchlink_attrs","onclick=\"window.location.href='JABATANAKTOR_search.aspx';return false;\"");
        smarty.Add("import_link",allow_import);
        smarty.Add("importlink_attrs","onclick=\"window.location.href='JABATANAKTOR_import.aspx';return false;\"");
    ...
    private void BuildForm()
    {
	    ...
	    for(int i = 0; i < collection.Count; ++ i)
        {
                        ...
                        if(beforeProcessRowListResult)
            {
			    ...
			    masterquery = "mastertable=JABATANAKTOR";
			    masterquery += "&masterkey1=" + this.Server.UrlEncode(collection[i].KODEJABATAN.ToString());
                masterID = collection[i].KODEJABATAN.ToString();
                //detail tables
                ...
                {
                    if (masterID != null && details.KODEJABATAN.ToString() == masterID)
                    {
                        ++AKTOR_cnt;
                    }
                }
            ...
            //	key fields
	            ...
	            string copylink="";
	            string keylink="";
	            keyblock +=  this.Server.UrlEncode(collection[i].KODEJABATAN.ToString());
	            editlink += "editid1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEJABATAN.ToString()));
	            copylink +="copyid1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEJABATAN.ToString()));
	            keylink +="&key1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEJABATAN.ToString()));

	            record["editlink_attrs"]="href=\"JABATANAKTOR_edit.aspx?" + editlink + "\" id=\"editlink" + recno.ToString() + "\"";
	            record["inlineeditlink_attrs"]= "href=\"JABATANAKTOR_edit.aspx?" + editlink + "\" onclick=\"return inlineEdit('" + recno.ToString() + "','" + editlink + "');\" id=\"ieditlink" + recno.ToString() + "\"";
	            record["copylink_attrs"]="href=\"JABATANAKTOR_add.aspx?" + copylink + "\" id=\"copylink" + recno.ToString() + "\"";
	            record["viewlink_attrs"]="href=\"JABATANAKTOR_view.aspx?"  + editlink + "\" id=\"viewlink" + recno.ToString() + "\"";
    ...
    private void GetData()
    {
        ...
        {
            collection = new Data.JABATANAKTORCollection();
            numrows = 0;
        }
        else
        {
        ...
        {
            try{
            AKTOR_masterids.Add(collection[i].KODEJABATAN.ToString());
            }
            catch
            {
            }
        }
        AKTOR_detailCollection = AKTOR_detailController.FetchForDetails("KODEJABATAN", AKTOR_masterids.ToArray(), OwnerColumn, OwnerID);
        ...
        if(allow_search)
        {
	        ...
	        else
	        {
	            ...
	            if(maxpages > 1)
	            {
                    ...
                    pagination += "{window.location='JABATANAKTOR_list.aspx?goto=' + nPageNumber;}</script>";
    ...
    private void ProcessSessionVariables()
    {
        ...
        smarty.Add("KODEJABATAN_orderlinkattrs", "href=\"JABATANAKTOR_list.aspx?orderby=aKODEJABATAN\"");
        smarty.Add("KODEJABATAN_fieldheader", true);
        smarty.Add("DESKRIPSI_orderlinkattrs", "href=\"JABATANAKTOR_list.aspx?orderby=aDESKRIPSI\"");
        ...
        if(tmp_orderby != null)
        {
	        ...
	        smarty[Control.GoodFieldName(order_field) + "_orderlinkattrs"] = "href=\"JABATANAKTOR_list.aspx?orderby=" + dir + Control.GoodFieldName(order_field) + "\"";
    ...
    private void Body()
    {
        ...
        "<form name=\"frmSearch\" method=\"GET\" action=\"JABATANAKTOR_list.aspx\">" + 
    ...
    private string GetIncludes()
    {
	    ...
	    includes.Append("var INLINE_EDIT_TABLE='JABATANAKTOR_edit.aspx';\r\n");
	    includes.Append("var INLINE_ADD_TABLE='JABATANAKTOR_add.aspx';\r\n");
	    includes.Append("var INLINE_VIEW_TABLE='JABATANAKTOR_view.aspx';\r\n");
	    includes.Append("var SUGGEST_TABLE='JABATANAKTOR_searchsuggest.aspx';\r\n");
	    includes.Append("var MASTER_PREVIEW_TABLE='JABATANAKTOR_masterpreview.aspx';\r\n");
        includes.Append("\n</script>\n");
