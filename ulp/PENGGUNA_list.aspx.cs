...
public partial class CPENGGUNA_List : AspNetRunnerPage 
{
    ...
    Data.PENGGUNACollection collection;
    ...
    protected void Page_Load( object sender,  System.EventArgs e)  
    {
        string output = string.Empty;
                        if (User != null && !string.IsNullOrEmpty("PENGGUNA"))
            {
                OwnerColumn = func.GetOwnerIDField("PENGGUNA");
            }
            rowinfo = array();
            bodyDict = array();
            rowinfo["data"] = list();
            ...
            output = func.BuildOutput(this, @"~\PENGGUNA_list.aspx", smarty);
    ...
    private void AddDisplayGridAttributes()
    {
        if(display_grid)
        {
            ...
            grid_block["begin"] = "<form method=\"POST\" action=\"PENGGUNA_list.aspx\" name=\"frmAdmin\" id=\"frmAdmin\">" +
        ...
        smarty.Add("recordspp_attrs","onchange=\"javascript: document.location='PENGGUNA_list.aspx?pagesize='+this.options[this.selectedIndex].value;\"");
    ...
    private void AddMasterDetailInfo()
    {
        ...
        //	include proper masterlist.aspx code
	        ...
	        string master = KELOMPOKPENGGUNAMasterList.CreateMasterTableInfo("PENGGUNA", keys, this);
	        smarty.Add("showmasterfile",master);
	        smarty.Add("backtomaster_link",true);
	        smarty.Add("mastertable_short","KELOMPOKPENGGUNA");
        }
    }
    ...
    private void AddLinkAttributes()
    {
	    ...
	    smarty.Add("printlink_attrs","onclick=\"window.open('PENGGUNA_print.aspx','wPrint');return false;\"");
	    smarty.Add("printalllink_attrs","onclick=\"window.open('PENGGUNA_print.aspx?all=1','wPrint');return false;\"");
	    smarty.Add("export_link", allow_export);
	    smarty.Add("exportlink_attrs","onclick=\"window.open('PENGGUNA_export.aspx','wExport');return false;\"");
		...
		"return true;" +
	    "document.forms.frmAdmin.action='PENGGUNA_print.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='PENGGUNA_list.aspx'; " +
	    ...
	    "if(!$('input[@type=checkbox][@checked][@name^=selection]').length)" +
		    "return true;" +
	    "document.forms.frmAdmin.action='PENGGUNA_export.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='PENGGUNA_list.aspx';  " +
	    "document.forms.frmAdmin.target='_self';\"");
	    ...
	    smarty.Add("addlink_attrs","onClick=\"window.location.href='PENGGUNA_add.aspx'\"");
	    smarty.Add("inlineaddlink_attrs","href=\"PENGGUNA_add.aspx\" onclick=\"return inlineAdd(newrecord_id++);\"");
        ...
        smarty.Add("KODEPENGGUNA_fieldheadercolumn",true);
        smarty.Add("KODEPENGGUNA_fieldcolumn",true);
        smarty.Add("KODEPENGGUNA_fieldfootercolumn",true);
        smarty.Add("NAMA_fieldheadercolumn",true);
        smarty.Add("NAMA_fieldcolumn",true);
        smarty.Add("NAMA_fieldfootercolumn",true);
        smarty.Add("KATAKUNCI_fieldheadercolumn",true);
        smarty.Add("KATAKUNCI_fieldcolumn",true);
        smarty.Add("KATAKUNCI_fieldfootercolumn",true);
        ...
        smarty.Add("asearchlink_attrs","onclick=\"window.location.href='PENGGUNA_search.aspx';return false;\"");
        smarty.Add("import_link",allow_import);
        smarty.Add("importlink_attrs","onclick=\"window.location.href='PENGGUNA_import.aspx';return false;\"");
    ...
    private void BuildForm()
    {
	    ...
	    for(int i = 0; i < collection.Count; ++ i)
        {
            ...
            //	key fields
	            ...
	            string copylink="";
	            string keylink="";
	            keyblock +=  this.Server.UrlEncode(collection[i].KODEPENGGUNA.ToString());
	            editlink += "editid1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEPENGGUNA.ToString()));
	            copylink +="copyid1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEPENGGUNA.ToString()));
	            keylink +="&key1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEPENGGUNA.ToString()));

	            record["editlink_attrs"]="href=\"PENGGUNA_edit.aspx?" + editlink + "\" id=\"editlink" + recno.ToString() + "\"";
	            record["inlineeditlink_attrs"]= "href=\"PENGGUNA_edit.aspx?" + editlink + "\" onclick=\"return inlineEdit('" + recno.ToString() + "','" + editlink + "');\" id=\"ieditlink" + recno.ToString() + "\"";
	            record["copylink_attrs"]="href=\"PENGGUNA_add.aspx?" + copylink + "\" id=\"copylink" + recno.ToString() + "\"";
	            record["viewlink_attrs"]="href=\"PENGGUNA_view.aspx?"  + editlink + "\" id=\"viewlink" + recno.ToString() + "\"";
	            record["checkbox_attrs"]="name=\"selection[]\" value=\"" + keyblock + "\" id=\"check" + recno.ToString() + "\"";

            string value;
            value = string.Empty;
    //	KODEPENGGUNA - 
            Control control_KODEPENGGUNA = new Control("KODEPENGGUNA", collection[i].KODEPENGGUNA, false, smarty, this.Request, builder, MODE.MODE_LIST);
			    ...
			    record["KODEPENGGUNA_value"]=value;
            value = string.Empty;
    //	NAMA - 
            ...
            value = string.Empty;
    //	KATAKUNCI - 
            Control control_KATAKUNCI = new Control("KATAKUNCI", collection[i].KATAKUNCI, false, smarty, this.Request, builder, MODE.MODE_LIST);
	                        value = control_KATAKUNCI.GetData();
			    value = control_KATAKUNCI.ProcessLargeText(value,"field=KATAKUNCI" + keylink,"",MODE.MODE_LIST);
			    record["KATAKUNCI_value"]=value;
            value = string.Empty;
    //	KODEKELOMPOK - 
            Control control_KODEKELOMPOK = new Control("KODEKELOMPOK", collection[i].KODEKELOMPOK, false, smarty, this.Request, builder, MODE.MODE_LIST);
			    ...
			    value=func.GetLookupValue(control_KODEKELOMPOK.FieldInfo);
                control_KODEKELOMPOK.Value = value;
    ...
    private void GetAdvancedSearchRows()
    {
        ...
        if(Mastertable=="KELOMPOKPENGGUNA")
        {
	        Asearchfor["KODEKELOMPOK"] = (string)this.Session[strTableName + "_masterkey1"];
            par.Add("KODEKELOMPOK", this.Session[strTableName + "_masterkey1"]);
        }
        string oCol = OwnerColumn;
        string oID = OwnerID;
    ...
    private void GetData()
    {
        ...
        {
            collection = new Data.PENGGUNACollection();
            numrows = 0;
        }
        else
        {
        ...
        if(allow_search)
        {
	        ...
	        else
	        {
	            ...
	            if(maxpages > 1)
	            {
                    ...
                    pagination += "{window.location='PENGGUNA_list.aspx?goto=' + nPageNumber;}</script>";
    ...
    private void ProcessSessionVariables()
    {
        ...
        smarty.Add("KODEPENGGUNA_orderlinkattrs", "href=\"PENGGUNA_list.aspx?orderby=aKODEPENGGUNA\"");
        smarty.Add("KODEPENGGUNA_fieldheader", true);
        smarty.Add("NAMA_orderlinkattrs", "href=\"PENGGUNA_list.aspx?orderby=aNAMA\"");
        smarty.Add("NAMA_fieldheader", true);
        smarty.Add("KATAKUNCI_orderlinkattrs", "href=\"PENGGUNA_list.aspx?orderby=aKATAKUNCI\"");
        smarty.Add("KATAKUNCI_fieldheader", true);
        smarty.Add("KODEKELOMPOK_orderlinkattrs", "href=\"PENGGUNA_list.aspx?orderby=aKODEKELOMPOK\"");
        ...
        if(tmp_orderby != null)
        {
	        ...
	        smarty[Control.GoodFieldName(order_field) + "_orderlinkattrs"] = "href=\"PENGGUNA_list.aspx?orderby=" + dir + Control.GoodFieldName(order_field) + "\"";
    ...
    private void Body()
    {
        ...
        "<form name=\"frmSearch\" method=\"GET\" action=\"PENGGUNA_list.aspx\">" + 
    ...
    private string GetIncludes()
    {
	    ...
	    includes.Append("var INLINE_EDIT_TABLE='PENGGUNA_edit.aspx';\r\n");
	    includes.Append("var INLINE_ADD_TABLE='PENGGUNA_add.aspx';\r\n");
	    includes.Append("var INLINE_VIEW_TABLE='PENGGUNA_view.aspx';\r\n");
	    includes.Append("var SUGGEST_TABLE='PENGGUNA_searchsuggest.aspx';\r\n");
	    includes.Append("var MASTER_PREVIEW_TABLE='PENGGUNA_masterpreview.aspx';\r\n");
        includes.Append("\n</script>\n");
        ...
        private void ProcessMasterTableOptions()
    {
        ...
        else
        {
            ...
            if(mtable=="KELOMPOKPENGGUNA")
            {
                ...
                smarty.Add("backtomasterlink_attrs","href=\"KELOMPOKPENGGUNA_list.aspx?a=return\"");
            }
    }
