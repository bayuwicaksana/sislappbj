...
public partial class CPOKJA_List : AspNetRunnerPage 
{
    ...
    Data.POKJACollection collection;
    ...
    private void AddDisplayGridAttributes()
    {
        if(display_grid)
        {
            ...
            grid_block["begin"] = "<form method=\"POST\" action=\"POKJA_list.aspx\" name=\"frmAdmin\" id=\"frmAdmin\">" +
        ...
        smarty.Add("recordspp_attrs","onchange=\"javascript: document.location='POKJA_list.aspx?pagesize='+this.options[this.selectedIndex].value;\"");
    ...
    private void AddLinkAttributes()
    {
	    ...
	    smarty.Add("printlink_attrs","onclick=\"window.open('POKJA_print.aspx','wPrint');return false;\"");
	    smarty.Add("printalllink_attrs","onclick=\"window.open('POKJA_print.aspx?all=1','wPrint');return false;\"");
	    smarty.Add("export_link", allow_export);
	    smarty.Add("exportlink_attrs","onclick=\"window.open('POKJA_export.aspx','wExport');return false;\"");
		...
		"return true;" +
	    "document.forms.frmAdmin.action='POKJA_print.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='POKJA_list.aspx'; " +
	    ...
	    "if(!$('input[@type=checkbox][@checked][@name^=selection]').length)" +
		    "return true;" +
	    "document.forms.frmAdmin.action='POKJA_export.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='POKJA_list.aspx';  " +
	    "document.forms.frmAdmin.target='_self';\"");
	    ...
	    smarty.Add("addlink_attrs","onClick=\"window.location.href='POKJA_add.aspx'\"");
	    smarty.Add("inlineaddlink_attrs","href=\"POKJA_add.aspx\" onclick=\"return inlineAdd(newrecord_id++);\"");
        ...
        smarty.Add("KODEPOKJA_fieldheadercolumn",true);
        smarty.Add("KODEPOKJA_fieldcolumn",true);
        smarty.Add("KODEPOKJA_fieldfootercolumn",true);
        smarty.Add("NAMA_fieldheadercolumn",true);
        smarty.Add("NAMA_fieldcolumn",true);
        smarty.Add("NAMA_fieldfootercolumn",true);
        smarty.Add("DESKRIPSSI_fieldheadercolumn",true);
        smarty.Add("DESKRIPSSI_fieldcolumn",true);
        smarty.Add("DESKRIPSSI_fieldfootercolumn",true);
        	
                display_grid = allow_search && rowsfound;

        smarty.Add("asearch_link",allow_search);
        smarty.Add("asearchlink_attrs","onclick=\"window.location.href='POKJA_search.aspx';return false;\"");
        smarty.Add("import_link",allow_import);
        smarty.Add("importlink_attrs","onclick=\"window.location.href='POKJA_import.aspx';return false;\"");
    ...
    private void BuildForm()
    {
	    ...
	    for(int i = 0; i < collection.Count; ++ i)
        {
            ...
            //	key fields
	            ...
	            string copylink="";
	            string keylink="";
	            keyblock +=  this.Server.UrlEncode(collection[i].KODEPOKJA.ToString());
	            editlink += "editid1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEPOKJA.ToString()));
	            copylink +="copyid1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEPOKJA.ToString()));
	            keylink +="&key1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEPOKJA.ToString()));

	            record["editlink_attrs"]="href=\"POKJA_edit.aspx?" + editlink + "\" id=\"editlink" + recno.ToString() + "\"";
	            record["inlineeditlink_attrs"]= "href=\"POKJA_edit.aspx?" + editlink + "\" onclick=\"return inlineEdit('" + recno.ToString() + "','" + editlink + "');\" id=\"ieditlink" + recno.ToString() + "\"";
	            record["copylink_attrs"]="href=\"POKJA_add.aspx?" + copylink + "\" id=\"copylink" + recno.ToString() + "\"";
	            record["viewlink_attrs"]="href=\"POKJA_view.aspx?"  + editlink + "\" id=\"viewlink" + recno.ToString() + "\"";
	            record["checkbox_attrs"]="name=\"selection[]\" value=\"" + keyblock + "\" id=\"check" + recno.ToString() + "\"";

            string value;
            value = string.Empty;
    //	KODEPOKJA - 
            Control control_KODEPOKJA = new Control("KODEPOKJA", collection[i].KODEPOKJA, false, smarty, this.Request, builder, MODE.MODE_LIST);
	                        value = control_KODEPOKJA.GetData();
			    value = control_KODEPOKJA.ProcessLargeText(value,"field=KODEPOKJA" + keylink,"",MODE.MODE_LIST);
			    record["KODEPOKJA_value"]=value;
            value = string.Empty;
    //	NAMA - 
            ...
            value = string.Empty;
    //	DESKRIPSSI - 
            Control control_DESKRIPSSI = new Control("DESKRIPSSI", collection[i].DESKRIPSSI, false, smarty, this.Request, builder, MODE.MODE_LIST);
	                        value = control_DESKRIPSSI.GetData();
			    value = control_DESKRIPSSI.ProcessLargeText(value,"field=DESKRIPSSI" + keylink,"",MODE.MODE_LIST);
			    record["DESKRIPSSI_value"]=value;
			    record["show"]=true;
    ...
    private void GetData()
    {
        ...
        {
            collection = new Data.POKJACollection();
            numrows = 0;
        }
        else
        {
        ...
        if(allow_search)
        {
	        ...
	        else
	        {
	            ...
	            if(maxpages > 1)
	            {
                    ...
                    pagination += "{window.location='POKJA_list.aspx?goto=' + nPageNumber;}</script>";
    ...
    private void ProcessSessionVariables()
    {
        ...
        smarty.Add("KODEPOKJA_orderlinkattrs", "href=\"POKJA_list.aspx?orderby=aKODEPOKJA\"");
        smarty.Add("KODEPOKJA_fieldheader", true);
        smarty.Add("NAMA_orderlinkattrs", "href=\"POKJA_list.aspx?orderby=aNAMA\"");
        smarty.Add("NAMA_fieldheader", true);
        smarty.Add("DESKRIPSSI_orderlinkattrs", "href=\"POKJA_list.aspx?orderby=aDESKRIPSSI\"");
        smarty.Add("DESKRIPSSI_fieldheader", true);
        ...
        if(tmp_orderby != null)
        {
	        ...
	        smarty[Control.GoodFieldName(order_field) + "_orderlinkattrs"] = "href=\"POKJA_list.aspx?orderby=" + dir + Control.GoodFieldName(order_field) + "\"";
    ...
    private void Body()
    {
        ...
        "<form name=\"frmSearch\" method=\"GET\" action=\"POKJA_list.aspx\">" + 
    ...
    private string GetIncludes()
    {
	    ...
	    includes.Append("var INLINE_EDIT_TABLE='POKJA_edit.aspx';\r\n");
	    includes.Append("var INLINE_ADD_TABLE='POKJA_add.aspx';\r\n");
	    includes.Append("var INLINE_VIEW_TABLE='POKJA_view.aspx';\r\n");
	    includes.Append("var SUGGEST_TABLE='POKJA_searchsuggest.aspx';\r\n");
	    includes.Append("var MASTER_PREVIEW_TABLE='POKJA_masterpreview.aspx';\r\n");
        includes.Append("\n</script>\n");
