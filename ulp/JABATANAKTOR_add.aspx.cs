...
public partial class CJABATANAKTOR_Add : AspNetRunnerPage 
{
    ...
    private void CopyRecord()
    {
        ...
        if(Request["copyid1"] != null || Request["editid1"] != null)
        {
	        ...
	        {
                item = Data.JABATANAKTOR.FetchByID(Request["copyid1"]);
                isCopy = true;
	        }
	        else
	        {
                 item = Data.JABATANAKTOR.FetchByID(Request["editid1"]);
                        ...
                        //	clear key fields
	            defvalues["KODEJABATAN"] = item.KODEJABATAN;
        }
    ...
    private void BuildBody()
    {
	    ...
	    if(inlineedit != ADD_MODE.ADD_ONTHEFLY)
	    {
		    ...
		    body["begin"] = includes + 
		    "<form name=\"editform\" encType=\"multipart/form-data\" method=\"post\" action=\"JABATANAKTOR_add.aspx\" " + onsubmit + ">" +
	    ...
	    else
	    {
		    ...
		    body["begin"]= "<form name=\"editform" + (string)Request["id"] + "\" encType=\"multipart/form-data\" method=\"post\" action=\"JABATANAKTOR_add.aspx\" " + onsubmit + " target=\"flyframe" + (string)Request["id"] + "\">" +
		    "<input type=hidden name=\"a\" value=\"added\">" + 
        ...
        if ( inlineedit != ADD_MODE.ADD_INLINE ) 
        {
            ...
            else
            {
		        ...
		        if(!string.IsNullOrEmpty(validatetype))
		        {
			        ...
			        if(inlineedit == ADD_MODE.ADD_ONTHEFLY)
                    {
				        linkdata += "define_fly('value_KODEJABATAN_" + (string)Request["id"] + "','" + validatetype + "');";
                    }
			        else
                    {
    ...
    private void Init()
    {
        ...
        if(inlineedit == ADD_MODE.ADD_INLINE)
        {
	        templatefile = "JABATANAKTOR_inline_add.aspx";
        }
        else
        {
	        templatefile = "JABATANAKTOR_add.aspx";
        }
    }
