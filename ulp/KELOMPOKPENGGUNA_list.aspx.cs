...
public partial class CKELOMPOKPENGGUNA_List : AspNetRunnerPage 
{
    ...
    Data.KELOMPOKPENGGUNACollection collection;

    Data.PENGGUNACollection PENGGUNA_detailCollection;

    protected void Page_Init( object sender,  System.EventArgs e)  
    {
        strTableName = "dbo.KELOMPOKPENGGUNA";
        strTableNameLocale = "dbo_KELOMPOKPENGGUNA";
    }

    protected void Page_Load( object sender,  System.EventArgs e)  
    {
        string output = string.Empty;
                        if (User != null && !string.IsNullOrEmpty("KELOMPOKPENGGUNA"))
            {
                OwnerColumn = func.GetOwnerIDField("KELOMPOKPENGGUNA");
            }
                        allow_PENGGUNA = func.CheckUserPermissions("dbo.PENGGUNA", "SA");
            rowinfo = array();
            bodyDict = array();
            rowinfo["data"] = list();
            ...
            output = func.BuildOutput(this, @"~\KELOMPOKPENGGUNA_list.aspx", smarty);
    ...
    private void AddDisplayGridAttributes()
    {
        if(display_grid)
        {
            ...
            grid_block["begin"] = "<form method=\"POST\" action=\"KELOMPOKPENGGUNA_list.aspx\" name=\"frmAdmin\" id=\"frmAdmin\">" +
        ...
        smarty.Add("recordspp_attrs","onchange=\"javascript: document.location='KELOMPOKPENGGUNA_list.aspx?pagesize='+this.options[this.selectedIndex].value;\"");
    ...
    private void AddLinkAttributes()
    {
	    ...
	    smarty.Add("printlink_attrs","onclick=\"window.open('KELOMPOKPENGGUNA_print.aspx','wPrint');return false;\"");
	    smarty.Add("printalllink_attrs","onclick=\"window.open('KELOMPOKPENGGUNA_print.aspx?all=1','wPrint');return false;\"");
	    smarty.Add("export_link", allow_export);
	    smarty.Add("exportlink_attrs","onclick=\"window.open('KELOMPOKPENGGUNA_export.aspx','wExport');return false;\"");
		...
		"return true;" +
	    "document.forms.frmAdmin.action='KELOMPOKPENGGUNA_print.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='KELOMPOKPENGGUNA_list.aspx'; " +
	    ...
	    "if(!$('input[@type=checkbox][@checked][@name^=selection]').length)" +
		    "return true;" +
	    "document.forms.frmAdmin.action='KELOMPOKPENGGUNA_export.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='KELOMPOKPENGGUNA_list.aspx';  " +
	    "document.forms.frmAdmin.target='_self';\"");
	    ...
	    smarty.Add("addlink_attrs","onClick=\"window.location.href='KELOMPOKPENGGUNA_add.aspx'\"");
	    smarty.Add("inlineaddlink_attrs","href=\"KELOMPOKPENGGUNA_add.aspx\" onclick=\"return inlineAdd(newrecord_id++);\"");
	    ...
	    smarty.Add("view_column",allow_search);
 	        ...
 	        smarty.Add("PENGGUNA_dtable_column", allow_PENGGUNA);


        smarty.Add("KODEKELOMPOK_fieldheadercolumn",true);
        smarty.Add("KODEKELOMPOK_fieldcolumn",true);
        smarty.Add("KODEKELOMPOK_fieldfootercolumn",true);
        ...
        smarty.Add("asearchlink_attrs","onclick=\"window.location.href='KELOMPOKPENGGUNA_search.aspx';return false;\"");
        smarty.Add("import_link",allow_import);
        smarty.Add("importlink_attrs","onclick=\"window.location.href='KELOMPOKPENGGUNA_import.aspx';return false;\"");
    ...
    private void BuildForm()
    {
	    ...
	    for(int i = 0; i < collection.Count; ++ i)
        {
                        ...
                        if(beforeProcessRowListResult)
            {
			    ...
			    masterquery = "mastertable=KELOMPOKPENGGUNA";
			    masterquery += "&masterkey1=" + this.Server.UrlEncode(collection[i].KODEKELOMPOK.ToString());
                masterID = collection[i].KODEKELOMPOK.ToString();
                //detail tables
	            record["PENGGUNA_dtable_link"]=true;
	            //record["PENGGUNA_dtablelink_attrs"]="onmouseover=\"RollDetailsLink.showPopup(this,'PENGGUNA_detailspreview.aspx'+this.href.substr(this.href.indexOf('?')));\" onmouseout=\"RollDetailsLink.hidePopup();\" href=\"PENGGUNA_list.aspx?" + masterquery + "\" id=\"master_PENGGUNA" + recno + "\"";

                dpreview=true;
                int PENGGUNA_cnt = 0;
                foreach (Data.PENGGUNA details in PENGGUNA_detailCollection)
                {
                    if (masterID != null && details.KODEKELOMPOK.ToString() == masterID)
                    {
                        ++PENGGUNA_cnt;
                    }
                }
	            	            if(PENGGUNA_cnt > 0)
                {
		            record["PENGGUNA_childcount"]=true;
                }
	            else
                {
		            dpreview=false;
                }
	            record["PENGGUNA_childnumber"]=PENGGUNA_cnt;
	                	        
                record["PENGGUNA_dtablelink_attrs"]="href=\"PENGGUNA_list.aspx?" + masterquery + "\" id=\"master_PENGGUNA" + recno.ToString() + "\"";
                if(dpreview)
	            {
	            		            record["PENGGUNA_dtablelink_attrs"] +=" onmouseover=\"RollDetailsLink.showPopup(this,'PENGGUNA_detailspreview.aspx'+this.href.substr(this.href.indexOf('?')));\" onmouseout=\"RollDetailsLink.hidePopup();\"";
	                            }
            ...
            //	key fields
	            ...
	            string copylink="";
	            string keylink="";
	            keyblock +=  this.Server.UrlEncode(collection[i].KODEKELOMPOK.ToString());
	            editlink += "editid1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEKELOMPOK.ToString()));
	            copylink +="copyid1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEKELOMPOK.ToString()));
	            keylink +="&key1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEKELOMPOK.ToString()));

	            record["editlink_attrs"]="href=\"KELOMPOKPENGGUNA_edit.aspx?" + editlink + "\" id=\"editlink" + recno.ToString() + "\"";
	            record["inlineeditlink_attrs"]= "href=\"KELOMPOKPENGGUNA_edit.aspx?" + editlink + "\" onclick=\"return inlineEdit('" + recno.ToString() + "','" + editlink + "');\" id=\"ieditlink" + recno.ToString() + "\"";
	            record["copylink_attrs"]="href=\"KELOMPOKPENGGUNA_add.aspx?" + copylink + "\" id=\"copylink" + recno.ToString() + "\"";
	            record["viewlink_attrs"]="href=\"KELOMPOKPENGGUNA_view.aspx?"  + editlink + "\" id=\"viewlink" + recno.ToString() + "\"";
	            record["checkbox_attrs"]="name=\"selection[]\" value=\"" + keyblock + "\" id=\"check" + recno.ToString() + "\"";

            string value;
            value = string.Empty;
    //	KODEKELOMPOK - 
            Control control_KODEKELOMPOK = new Control("KODEKELOMPOK", collection[i].KODEKELOMPOK, false, smarty, this.Request, builder, MODE.MODE_LIST);
			    ...
			    record["KODEKELOMPOK_value"]=value;
            value = string.Empty;
    //	DESKRIPSI - 
    ...
    private void ProcessSessionVariables()
    {
        ...
        smarty.Add("KODEKELOMPOK_orderlinkattrs", "href=\"KELOMPOKPENGGUNA_list.aspx?orderby=aKODEKELOMPOK\"");
        smarty.Add("KODEKELOMPOK_fieldheader", true);
        smarty.Add("DESKRIPSI_orderlinkattrs", "href=\"KELOMPOKPENGGUNA_list.aspx?orderby=aDESKRIPSI\"");
        ...
        if(tmp_orderby != null)
        {
	        ...
	        smarty[Control.GoodFieldName(order_field) + "_orderlinkattrs"] = "href=\"KELOMPOKPENGGUNA_list.aspx?orderby=" + dir + Control.GoodFieldName(order_field) + "\"";
    ...
    private void Body()
    {
        ...
        "<form name=\"frmSearch\" method=\"GET\" action=\"KELOMPOKPENGGUNA_list.aspx\">" + 
    ...
    private string GetIncludes()
    {
	    ...
	    includes.Append("var INLINE_EDIT_TABLE='KELOMPOKPENGGUNA_edit.aspx';\r\n");
	    includes.Append("var INLINE_ADD_TABLE='KELOMPOKPENGGUNA_add.aspx';\r\n");
	    includes.Append("var INLINE_VIEW_TABLE='KELOMPOKPENGGUNA_view.aspx';\r\n");
	    includes.Append("var SUGGEST_TABLE='KELOMPOKPENGGUNA_searchsuggest.aspx';\r\n");
	    includes.Append("var MASTER_PREVIEW_TABLE='KELOMPOKPENGGUNA_masterpreview.aspx';\r\n");
        includes.Append("\n</script>\n");
