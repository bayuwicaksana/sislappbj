...
public partial class CDOKUMEN_Edit : AspNetRunnerPage 
{
    ...
    private void SaveData()
    {
        if (RequestAction != "edited")
        {
            item = new Data.DOKUMEN();
        }

        Data.DOKUMEN emptyItem = new Data.DOKUMEN();
        ...
        //
        //item.KODEDOKUMEN =  Convert.ToString(Request["editid1"]);
        //
        string idx = string.Empty;
        idx = "1";
                                               item.KODEDOKUMEN =  Convert.ToString(Request["editid" + idx]);
    ...
    private void BuildForm()
    {
        ...
        string keylink="";
	    keylink +="&key1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(item.KODEDOKUMEN.ToString()));

        if(RequestAction == "edited" && inlineedit)
        {
            string masterquery = string.Empty;
	        masterquery="mastertable=dbo%2EDOKUMEN";
	        masterquery += "&masterkey1=" + this.Server.UrlEncode(item.KODEDOKUMEN.ToString());
	        showDetailKeys["KELENGKAPAN"] = masterquery;
        	
	        showKeys.Add(Control.HTMLEncodeSpecialChars(item.KODEDOKUMEN.ToString()));
    ...
    private void BuildBody()
    {
		...
		body["begin"] = includes + 
		"<form name=\"editform\" encType=\"multipart/form-data\" method=\"post\" action=\"DOKUMEN_edit.aspx\" " + onsubmit + ">" +
        ...
        body["begin"] +="<input type=\"hidden\" name=\"editid1\" value=\""+ Control.HTMLEncodeSpecialChars(keys["KODEDOKUMEN"].ToString()) + "\">";
	    	    smarty.Add("show_key1", Control.HTMLEncodeSpecialChars(control_KODEDOKUMEN.GetData()));

        smarty.Add("backbutton_attrs","onclick=\"window.location.href='DOKUMEN_list.aspx?a=return'\"");
	    smarty.Add("save_button",true);
	    smarty.Add("reset_button",true);
	    smarty.Add("back_button",true);
	    
        showKeys.Add(Server.UrlEncode(keys["KODEDOKUMEN"].ToString()));
    }
    ...
    private string GetIncludes()
    {
        ...
        if ( !inlineedit ) 
        {
	        ...
	        includes.Append( "</script>\r\n");
		        ...
		        {
			        bodyonload += "define('value_KODEDOKUMEN','" + validatetype + "','KODE DOKUMEN');";
		        }
    ...
    private bool Init()
    {
        ...
        if(inlineedit)
        {
	        templatefile = "DOKUMEN_inline_edit.aspx";
        }
        else
        {
	        templatefile = "DOKUMEN_edit.aspx";
        }

        keys["KODEDOKUMEN"] = Request["editid1"];
        if(keys.Count > 1)
        {
        ...
        {
            item = Data.DOKUMEN.FetchByID(Request["editid1"]);
        }
        ...
        if(item != null)
        {
            ...
            if(!editable)
            {
                output.Append("<p>" + "You don't have permissions to access this table" + " <a href=\"DOKUMEN_list.aspx?a=return\">back</a>");
                return false;
            }
        }
        else
        {
            this.Server.Transfer("~/DOKUMEN_list.aspx?a=return");
        }
