...
public partial class CSTATUSPBJ_Edit : AspNetRunnerPage 
{
    ...
    private void BuildForm()
    {
        ...
        if(RequestAction == "edited" && inlineedit)
        {
	        ...
	        masterquery="mastertable=dbo%2ESTATUSPBJ";
	        masterquery += "&masterkey1=" + this.Server.UrlEncode(item.KODESTATUS.ToString());
	        showDetailKeys["PBJ"] = masterquery;
        	
	        showKeys.Add(Control.HTMLEncodeSpecialChars(item.KODESTATUS.ToString()));
        ...
        else
        {
            ...
            if(defvalues.ContainsKey("KODESTATUS"))
            {
                control_KODESTATUS = new Control("KODESTATUS", defvalues["KODESTATUS"], false, smarty, this.Request, builder, ctrlMode);
            }
            else
            {
                control_KODESTATUS = new Control("KODESTATUS", item.KODESTATUS, false, smarty, this.Request, builder, ctrlMode);
            }
            ...
            if(defvalues.ContainsKey("URUTAN"))
            {
                control_URUTAN = new Control("URUTAN", defvalues["URUTAN"], false, smarty, this.Request, builder, ctrlMode);
            }
            else
            {
                control_URUTAN = new Control("URUTAN", item.URUTAN, false, smarty, this.Request, builder, ctrlMode);
            }
    ...
    private void BuildBody()
    {
		...
		body["begin"] = includes + 
		"<form name=\"editform\" encType=\"multipart/form-data\" method=\"post\" action=\"STATUSPBJ_edit.aspx\" " + onsubmit + ">" +
		"<input type=hidden name=\"a\" value=\"edited\">";
        Control control_KODESTATUS = new Control("KODESTATUS", item.KODESTATUS, false, smarty, this.Request, builder, MODE.MODE_LIST);
        body["begin"] +="<input type=\"hidden\" name=\"editid1\" value=\""+ Control.HTMLEncodeSpecialChars(keys["KODESTATUS"].ToString()) + "\">";
	    	    smarty.Add("show_key1", Control.HTMLEncodeSpecialChars(control_KODESTATUS.GetData()));
        ...
        showKeys.Add(Server.UrlEncode(keys["KODESTATUS"].ToString()));
    }
    ...
    private string GetIncludes()
    {
        ...
        if ( !inlineedit ) 
        {
	        ...
	        includes.Append( "</script>\r\n");
		        ...
		        {
			        bodyonload += "define('value_KODESTATUS','" + validatetype + "','KODE STATUS');";
		        }
		        ...
		        if(!string.IsNullOrEmpty(validatetype))
		        {
			        bodyonload += "define('value_URUTAN','" + validatetype + "','URUTAN');";
		        }
