...
public class STATUSPBJMasterPrint
{
    public static string DisplayMasterTableInfo(string sourcetable, IList<object> masterkeys, System.Web.UI.Page page)
    {
        ...
        STATUSPBJCollection collection = controller.FetchByID(masterkeys[0]);

	    string keylink = string.Empty;
        if(collection != null && collection.Count > 0)
        {
	    keylink += "&key1=" + Control.HTMLEncodeSpecialChars(HttpUtility.UrlEncode(collection[0].KODESTATUS.ToString()));
        }
        	
        string value="";

        Control control_KODESTATUS = new Control("KODESTATUS", collection[0].KODESTATUS, false, smarty, page.Request, builder, MODE.MODE_LIST);
        //	KODESTATUS - 
        ...
        Control control_URUTAN = new Control("URUTAN", collection[0].URUTAN, false, smarty, page.Request, builder, MODE.MODE_LIST);
        //	URUTAN - 
            ...
            return func.BuildOutput(page, @"~\STATUSPBJ_masterprint.aspx", smarty);
    }
}
