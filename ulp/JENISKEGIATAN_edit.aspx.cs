...
public partial class CJENISKEGIATAN_Edit : AspNetRunnerPage 
{
    ...
    private void SaveData()
    {
        if (RequestAction != "edited")
        {
            item = new Data.JENISKEGIATAN();
        }

        Data.JENISKEGIATAN emptyItem = new Data.JENISKEGIATAN();
        ...
        //
        //item.KODEJENISKEGIATAN =  Convert.ToString(Request["editid1"]);
        //
        string idx = string.Empty;
        idx = "1";
                                               item.KODEJENISKEGIATAN =  Convert.ToString(Request["editid" + idx]);
    ...
    private void BuildForm()
    {
        ...
        string keylink="";
	    keylink +="&key1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(item.KODEJENISKEGIATAN.ToString()));

        if(RequestAction == "edited" && inlineedit)
        {
            string masterquery = string.Empty;
	        masterquery="mastertable=dbo%2EJENISKEGIATAN";
	        masterquery += "&masterkey1=" + this.Server.UrlEncode(item.KODEJENISKEGIATAN.ToString());
	        showDetailKeys["PBJ"] = masterquery;
	        masterquery="mastertable=dbo%2EJENISKEGIATAN";
	        masterquery += "&masterkey1=" + this.Server.UrlEncode(item.KODEJENISKEGIATAN.ToString());
	        showDetailKeys["KELENGKAPAN"] = masterquery;
        	
	        showKeys.Add(Control.HTMLEncodeSpecialChars(item.KODEJENISKEGIATAN.ToString()));
    ...
    private void BuildBody()
    {
		...
		body["begin"] = includes + 
		"<form name=\"editform\" encType=\"multipart/form-data\" method=\"post\" action=\"JENISKEGIATAN_edit.aspx\" " + onsubmit + ">" +
        ...
        body["begin"] +="<input type=\"hidden\" name=\"editid1\" value=\""+ Control.HTMLEncodeSpecialChars(keys["KODEJENISKEGIATAN"].ToString()) + "\">";
	    	    smarty.Add("show_key1", Control.HTMLEncodeSpecialChars(control_KODEJENISKEGIATAN.GetData()));

        smarty.Add("backbutton_attrs","onclick=\"window.location.href='JENISKEGIATAN_list.aspx?a=return'\"");
	    smarty.Add("save_button",true);
	    smarty.Add("reset_button",true);
	    smarty.Add("back_button",true);
	    
        showKeys.Add(Server.UrlEncode(keys["KODEJENISKEGIATAN"].ToString()));
    }
    ...
    private string GetIncludes()
    {
        ...
        if ( !inlineedit ) 
        {
	        ...
	        includes.Append( "</script>\r\n");
		        ...
		        {
			        bodyonload += "define('value_KODEJENISKEGIATAN','" + validatetype + "','KODE JENIS KEGIATAN');";
		        }
    ...
    private bool Init()
    {
        ...
        if(inlineedit)
        {
	        templatefile = "JENISKEGIATAN_inline_edit.aspx";
        }
        else
        {
	        templatefile = "JENISKEGIATAN_edit.aspx";
        }

        keys["KODEJENISKEGIATAN"] = Request["editid1"];
        if(keys.Count > 1)
        {
        ...
        {
            item = Data.JENISKEGIATAN.FetchByID(Request["editid1"]);
        }
        ...
        if(item != null)
        {
            ...
            if(!editable)
            {
                output.Append("<p>" + "You don't have permissions to access this table" + " <a href=\"JENISKEGIATAN_list.aspx?a=return\">back</a>");
                return false;
            }
        }
        else
        {
            this.Server.Transfer("~/JENISKEGIATAN_list.aspx?a=return");
        }
