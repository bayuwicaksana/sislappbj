...
public partial class CSTATUSPBJ_Add : AspNetRunnerPage 
{
    ...
    private void BuildForm()
    {
        ...
        if(RequestAction == "added")
        {
	        ...
	        //	DESKRIPSI - 
		        ...
		        showFields.Add("DESKRIPSI");
		        		        showRawValues.Add(string.Empty);
            value="";
            Control control_URUTAN = new Control("URUTAN", item.URUTAN, false, smarty, this.Request, builder, MODE.MODE_LIST);
	        ////////////////////////////////////////////
	        //	URUTAN - 
        ...
        else
        {
            ...
            object KODESTATUS_value = null;
            if(defvalues.ContainsKey("KODESTATUS"))
            {
                KODESTATUS_value = defvalues["KODESTATUS"];
            }
            else if(isCopy)
            {
                KODESTATUS_value = item.KODESTATUS;
            }
            if(inlineedit == ADD_MODE.ADD_INLINE)
            {
            ...
            Control edit_control_KODESTATUS = null;

            if(!string.IsNullOrEmpty(Mastertable) && (detailkeys.ContainsKey("KODESTATUS")))
            {
                edit_control_KODESTATUS = new Control("KODESTATUS", detailkeys["KODESTATUS"], false, smarty, this.Request, builder, ctrlMode);
            }
            else
            {
                                edit_control_KODESTATUS = new Control("KODESTATUS", KODESTATUS_value, false, smarty, this.Request, builder, ctrlMode);
            }

                                    smarty.Add("KODESTATUS_editcontrol", edit_control_KODESTATUS.BuildEditControl());
            ...
            object URUTAN_value = null;
            if(defvalues.ContainsKey("URUTAN"))
            {
                URUTAN_value = defvalues["URUTAN"];
            }
            else if(isCopy)
            {
                URUTAN_value = item.URUTAN;
            }
            if(inlineedit == ADD_MODE.ADD_INLINE)
            {
                ctrlMode = MODE.MODE_INLINE_ADD;
            ...
            Control edit_control_URUTAN = null;

            if(!string.IsNullOrEmpty(Mastertable) && (detailkeys.ContainsKey("URUTAN")))
            {
                edit_control_URUTAN = new Control("URUTAN", detailkeys["URUTAN"], false, smarty, this.Request, builder, ctrlMode);
            }
            else
            {
                                edit_control_URUTAN = new Control("URUTAN", URUTAN_value, false, smarty, this.Request, builder, ctrlMode);
            }

                                    smarty.Add("URUTAN_editcontrol", edit_control_URUTAN.BuildEditControl());
        }
    }
    ...
    private void BuildBody()
    {
	    ...
	    if(inlineedit != ADD_MODE.ADD_ONTHEFLY)
	    {
		    ...
		    body["begin"] = includes + 
		    "<form name=\"editform\" encType=\"multipart/form-data\" method=\"post\" action=\"STATUSPBJ_add.aspx\" " + onsubmit + ">" +
	    ...
	    else
	    {
		    ...
		    body["begin"]= "<form name=\"editform" + (string)Request["id"] + "\" encType=\"multipart/form-data\" method=\"post\" action=\"STATUSPBJ_add.aspx\" " + onsubmit + " target=\"flyframe" + (string)Request["id"] + "\">" +
		    "<input type=hidden name=\"a\" value=\"added\">" + 
        ...
        if ( inlineedit != ADD_MODE.ADD_INLINE ) 
        {
            ...
            else
            {
		        ...
		        if(!string.IsNullOrEmpty(validatetype))
		        {
			        ...
			        if(inlineedit == ADD_MODE.ADD_ONTHEFLY)
                    {
				        linkdata += "define_fly('value_KODESTATUS_" + (string)Request["id"] + "','" + validatetype + "');";
                    }
			        else
                    {
		        ...
		        if(!string.IsNullOrEmpty(validatetype))
		        {
			        ...
			        if(inlineedit == ADD_MODE.ADD_ONTHEFLY)
                    {
				        linkdata += "define_fly('value_URUTAN_" + (string)Request["id"] + "','" + validatetype + "');";
                    }
			        else
                    {
    ...
    private void Init()
    {
        ...
        if(inlineedit == ADD_MODE.ADD_INLINE)
        {
	        templatefile = "STATUSPBJ_inline_add.aspx";
        }
        else
        {
	        templatefile = "STATUSPBJ_add.aspx";
        }
    }
