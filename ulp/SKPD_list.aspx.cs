...
public partial class CSKPD_List : AspNetRunnerPage 
{
    ...
    Data.SKPDCollection collection;
    ...
    protected void Page_Load( object sender,  System.EventArgs e)  
    {
        string output = string.Empty;
                        if (User != null && !string.IsNullOrEmpty("SKPD"))
            {
                OwnerColumn = func.GetOwnerIDField("SKPD");
            }
            ...
            output = func.BuildOutput(this, @"~\SKPD_list.aspx", smarty);
    ...
    private void AddDisplayGridAttributes()
    {
        if(display_grid)
        {
            ...
            grid_block["begin"] = "<form method=\"POST\" action=\"SKPD_list.aspx\" name=\"frmAdmin\" id=\"frmAdmin\">" +
        ...
        smarty.Add("recordspp_attrs","onchange=\"javascript: document.location='SKPD_list.aspx?pagesize='+this.options[this.selectedIndex].value;\"");
    ...
    private void AddLinkAttributes()
    {
	    ...
	    smarty.Add("printlink_attrs","onclick=\"window.open('SKPD_print.aspx','wPrint');return false;\"");
	    smarty.Add("printalllink_attrs","onclick=\"window.open('SKPD_print.aspx?all=1','wPrint');return false;\"");
	    smarty.Add("export_link", allow_export);
	    smarty.Add("exportlink_attrs","onclick=\"window.open('SKPD_export.aspx','wExport');return false;\"");
		...
		"return true;" +
	    "document.forms.frmAdmin.action='SKPD_print.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='SKPD_list.aspx'; " +
	    ...
	    "if(!$('input[@type=checkbox][@checked][@name^=selection]').length)" +
		    "return true;" +
	    "document.forms.frmAdmin.action='SKPD_export.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='SKPD_list.aspx';  " +
	    "document.forms.frmAdmin.target='_self';\"");
	    ...
	    smarty.Add("addlink_attrs","onClick=\"window.location.href='SKPD_add.aspx'\"");
	    smarty.Add("inlineaddlink_attrs","href=\"SKPD_add.aspx\" onclick=\"return inlineAdd(newrecord_id++);\"");
        ...
        smarty.Add("asearchlink_attrs","onclick=\"window.location.href='SKPD_search.aspx';return false;\"");
        smarty.Add("import_link",allow_import);
        smarty.Add("importlink_attrs","onclick=\"window.location.href='SKPD_import.aspx';return false;\"");
    ...
    private void BuildForm()
    {
	    ...
	    for(int i = 0; i < collection.Count; ++ i)
        {
                        ...
                        if(beforeProcessRowListResult)
            {
			    ...
			    masterquery = "mastertable=SKPD";
			    masterquery += "&masterkey1=" + this.Server.UrlEncode(collection[i].KODESKPD.ToString());
                masterID = collection[i].KODESKPD.ToString();
                //detail tables
                ...
                {
                    if (masterID != null && details.KODESKPD.ToString() == masterID)
                    {
                        ++PBJ_cnt;
                    }
                }
            ...
            //	key fields
	            ...
	            string copylink="";
	            string keylink="";
	            keyblock +=  this.Server.UrlEncode(collection[i].KODESKPD.ToString());
	            editlink += "editid1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODESKPD.ToString()));
	            copylink +="copyid1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODESKPD.ToString()));
	            keylink +="&key1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODESKPD.ToString()));

	            record["editlink_attrs"]="href=\"SKPD_edit.aspx?" + editlink + "\" id=\"editlink" + recno.ToString() + "\"";
	            record["inlineeditlink_attrs"]= "href=\"SKPD_edit.aspx?" + editlink + "\" onclick=\"return inlineEdit('" + recno.ToString() + "','" + editlink + "');\" id=\"ieditlink" + recno.ToString() + "\"";
	            record["copylink_attrs"]="href=\"SKPD_add.aspx?" + copylink + "\" id=\"copylink" + recno.ToString() + "\"";
	            record["viewlink_attrs"]="href=\"SKPD_view.aspx?"  + editlink + "\" id=\"viewlink" + recno.ToString() + "\"";
    ...
    private void GetData()
    {
        ...
        {
            collection = new Data.SKPDCollection();
            numrows = 0;
        }
        else
        {
        ...
        {
            try{
            PBJ_masterids.Add(collection[i].KODESKPD.ToString());
            }
            catch
            {
            }
        }
        PBJ_detailCollection = PBJ_detailController.FetchForDetails("KODESKPD", PBJ_masterids.ToArray(), OwnerColumn, OwnerID);
        ...
        if(allow_search)
        {
	        ...
	        else
	        {
	            ...
	            if(maxpages > 1)
	            {
                    ...
                    pagination += "{window.location='SKPD_list.aspx?goto=' + nPageNumber;}</script>";
    ...
    private void ProcessSessionVariables()
    {
        ...
        smarty.Add("KODESKPD_orderlinkattrs", "href=\"SKPD_list.aspx?orderby=aKODESKPD\"");
        smarty.Add("KODESKPD_fieldheader", true);
        smarty.Add("DESKRIPSI_orderlinkattrs", "href=\"SKPD_list.aspx?orderby=aDESKRIPSI\"");
        smarty.Add("DESKRIPSI_fieldheader", true);
        smarty.Add("ALAMAT_orderlinkattrs", "href=\"SKPD_list.aspx?orderby=aALAMAT\"");
        ...
        if(tmp_orderby != null)
        {
	        ...
	        smarty[Control.GoodFieldName(order_field) + "_orderlinkattrs"] = "href=\"SKPD_list.aspx?orderby=" + dir + Control.GoodFieldName(order_field) + "\"";
    ...
    private void Body()
    {
        ...
        "<form name=\"frmSearch\" method=\"GET\" action=\"SKPD_list.aspx\">" + 
    ...
    private string GetIncludes()
    {
	    ...
	    includes.Append("var INLINE_EDIT_TABLE='SKPD_edit.aspx';\r\n");
	    includes.Append("var INLINE_ADD_TABLE='SKPD_add.aspx';\r\n");
	    includes.Append("var INLINE_VIEW_TABLE='SKPD_view.aspx';\r\n");
	    includes.Append("var SUGGEST_TABLE='SKPD_searchsuggest.aspx';\r\n");
	    includes.Append("var MASTER_PREVIEW_TABLE='SKPD_masterpreview.aspx';\r\n");
        includes.Append("\n</script>\n");
