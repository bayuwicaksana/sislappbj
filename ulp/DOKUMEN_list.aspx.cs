...
public partial class CDOKUMEN_List : AspNetRunnerPage 
{
    ...
    Data.DOKUMENController controller = new Data.DOKUMENController();
    Data.DOKUMENCollection collection;

    Data.KELENGKAPANCollection KELENGKAPAN_detailCollection;

    protected void Page_Init( object sender,  System.EventArgs e)  
    {
        strTableName = "dbo.DOKUMEN";
        strTableNameLocale = "dbo_DOKUMEN";
    }
    ...
    private void AddDisplayGridAttributes()
    {
        if(display_grid)
        {
            ...
            grid_block["begin"] = "<form method=\"POST\" action=\"DOKUMEN_list.aspx\" name=\"frmAdmin\" id=\"frmAdmin\">" +
        ...
        smarty.Add("recordspp_attrs","onchange=\"javascript: document.location='DOKUMEN_list.aspx?pagesize='+this.options[this.selectedIndex].value;\"");
    ...
    private void AddLinkAttributes()
    {
	    ...
	    smarty.Add("printlink_attrs","onclick=\"window.open('DOKUMEN_print.aspx','wPrint');return false;\"");
	    smarty.Add("printalllink_attrs","onclick=\"window.open('DOKUMEN_print.aspx?all=1','wPrint');return false;\"");
	    smarty.Add("export_link", allow_export);
	    smarty.Add("exportlink_attrs","onclick=\"window.open('DOKUMEN_export.aspx','wExport');return false;\"");
		...
		"return true;" +
	    "document.forms.frmAdmin.action='DOKUMEN_print.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='DOKUMEN_list.aspx'; " +
	    ...
	    "if(!$('input[@type=checkbox][@checked][@name^=selection]').length)" +
		    "return true;" +
	    "document.forms.frmAdmin.action='DOKUMEN_export.aspx';" +
	    "document.forms.frmAdmin.target='_blank';" +
	    "document.forms.frmAdmin.submit(); " +
	    "document.forms.frmAdmin.action='DOKUMEN_list.aspx';  " +
	    "document.forms.frmAdmin.target='_self';\"");
	    ...
	    smarty.Add("addlink_attrs","onClick=\"window.location.href='DOKUMEN_add.aspx'\"");
	    smarty.Add("inlineaddlink_attrs","href=\"DOKUMEN_add.aspx\" onclick=\"return inlineAdd(newrecord_id++);\"");
        ...
        smarty.Add("asearchlink_attrs","onclick=\"window.location.href='DOKUMEN_search.aspx';return false;\"");
        smarty.Add("import_link",allow_import);
        smarty.Add("importlink_attrs","onclick=\"window.location.href='DOKUMEN_import.aspx';return false;\"");
    ...
    private void BuildForm()
    {
	    ...
	    for(int i = 0; i < collection.Count; ++ i)
        {
                        ...
                        if(beforeProcessRowListResult)
            {
			    ...
			    masterquery = "mastertable=DOKUMEN";
			    masterquery += "&masterkey1=" + this.Server.UrlEncode(collection[i].KODEDOKUMEN.ToString());
                masterID = collection[i].KODEDOKUMEN.ToString();
                //detail tables
                ...
                {
                    if (masterID != null && details.KODEDOKUMEN.ToString() == masterID)
                    {
                        ++KELENGKAPAN_cnt;
                    }
                }
            ...
            //	key fields
	            ...
	            string copylink="";
	            string keylink="";
	            keyblock +=  this.Server.UrlEncode(collection[i].KODEDOKUMEN.ToString());
	            editlink += "editid1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEDOKUMEN.ToString()));
	            copylink +="copyid1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEDOKUMEN.ToString()));
	            keylink +="&key1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(collection[i].KODEDOKUMEN.ToString()));

	            record["editlink_attrs"]="href=\"DOKUMEN_edit.aspx?" + editlink + "\" id=\"editlink" + recno.ToString() + "\"";
	            record["inlineeditlink_attrs"]= "href=\"DOKUMEN_edit.aspx?" + editlink + "\" onclick=\"return inlineEdit('" + recno.ToString() + "','" + editlink + "');\" id=\"ieditlink" + recno.ToString() + "\"";
	            record["copylink_attrs"]="href=\"DOKUMEN_add.aspx?" + copylink + "\" id=\"copylink" + recno.ToString() + "\"";
	            record["viewlink_attrs"]="href=\"DOKUMEN_view.aspx?"  + editlink + "\" id=\"viewlink" + recno.ToString() + "\"";
    ...
    private void ProcessSessionVariables()
    {
        ...
        smarty.Add("KODEDOKUMEN_orderlinkattrs", "href=\"DOKUMEN_list.aspx?orderby=aKODEDOKUMEN\"");
        smarty.Add("KODEDOKUMEN_fieldheader", true);
        smarty.Add("DESKRIPSI_orderlinkattrs", "href=\"DOKUMEN_list.aspx?orderby=aDESKRIPSI\"");
        ...
        if(tmp_orderby != null)
        {
	        ...
	        smarty[Control.GoodFieldName(order_field) + "_orderlinkattrs"] = "href=\"DOKUMEN_list.aspx?orderby=" + dir + Control.GoodFieldName(order_field) + "\"";
    ...
    private void Body()
    {
        ...
        "<form name=\"frmSearch\" method=\"GET\" action=\"DOKUMEN_list.aspx\">" + 
    ...
    private string GetIncludes()
    {
	    ...
	    includes.Append("var INLINE_EDIT_TABLE='DOKUMEN_edit.aspx';\r\n");
	    includes.Append("var INLINE_ADD_TABLE='DOKUMEN_add.aspx';\r\n");
	    includes.Append("var INLINE_VIEW_TABLE='DOKUMEN_view.aspx';\r\n");
	    includes.Append("var SUGGEST_TABLE='DOKUMEN_searchsuggest.aspx';\r\n");
	    includes.Append("var MASTER_PREVIEW_TABLE='DOKUMEN_masterpreview.aspx';\r\n");
        includes.Append("\n</script>\n");
