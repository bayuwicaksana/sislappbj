...
public partial class CKELOMPOKPENGGUNA_Edit : AspNetRunnerPage 
{
    ...
    Data.KELOMPOKPENGGUNA item = null;
    ...
    private void BuildForm()
    {
        ...
        string keylink="";
	    keylink +="&key1=" + Control.HTMLEncodeSpecialChars(this.Server.UrlEncode(item.KODEKELOMPOK.ToString()));

        if(RequestAction == "edited" && inlineedit)
        {
            string masterquery = string.Empty;
	        masterquery="mastertable=dbo%2EKELOMPOKPENGGUNA";
	        masterquery += "&masterkey1=" + this.Server.UrlEncode(item.KODEKELOMPOK.ToString());
	        showDetailKeys["PENGGUNA"] = masterquery;
        	
	        showKeys.Add(Control.HTMLEncodeSpecialChars(item.KODEKELOMPOK.ToString()));
        ...
        else
        {
            ...
            {
                control_KODEKELOMPOK = new Control("KODEKELOMPOK", defvalues["KODEKELOMPOK"], false, smarty, this.Request, builder, ctrlMode);
            }
            else
            {
                control_KODEKELOMPOK = new Control("KODEKELOMPOK", item.KODEKELOMPOK, false, smarty, this.Request, builder, ctrlMode);
            }
    ...
    private void BuildBody()
    {
		...
		body["begin"] = includes + 
		"<form name=\"editform\" encType=\"multipart/form-data\" method=\"post\" action=\"KELOMPOKPENGGUNA_edit.aspx\" " + onsubmit + ">" +
        ...
        body["begin"] +="<input type=\"hidden\" name=\"editid1\" value=\""+ Control.HTMLEncodeSpecialChars(keys["KODEKELOMPOK"].ToString()) + "\">";
	    	    smarty.Add("show_key1", Control.HTMLEncodeSpecialChars(control_KODEKELOMPOK.GetData()));

        smarty.Add("backbutton_attrs","onclick=\"window.location.href='KELOMPOKPENGGUNA_list.aspx?a=return'\"");
	    smarty.Add("save_button",true);
	    smarty.Add("reset_button",true);
	    smarty.Add("back_button",true);
	    
        showKeys.Add(Server.UrlEncode(keys["KODEKELOMPOK"].ToString()));
    }
    ...
    private string GetIncludes()
    {
        ...
        if ( !inlineedit ) 
        {
	        ...
	        includes.Append( "</script>\r\n");
		        ...
		        {
			        bodyonload += "define('value_KODEKELOMPOK','" + validatetype + "','KODE KELOMPOK');";
		        }
